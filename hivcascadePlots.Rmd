...
# Removed linked and retained from hivcascade
if (excludeCare) {
  hivcascade <- filter(hivcascade, !(stage %in% c("linked", "retained")))
}
...
if (excludeCare) {
  levels(resultsAll$stage) <- c("Living with HIV", "Receiving ART", 
                                "Diagnosed", "Suppressed virus")
  resultsAll <- within(resultsAll, stage <- factor(stage, levels(stage)[c(1, 3, 2, 4)]))
} else {
...
```{r nswcascade, echo = FALSE, messages = FALSE, include=FALSE}
# Create bar chart of current year's cascade for NSW for steering committee meeting
resultsAllnsw <- filter(hivcascade, population == "nsw" & year == 2014)
...
### Plot the bar chart
plotbarNSW <- ggplot(data = resultsAllnsw, aes(x = stage, y = value)) + 
  geom_bar(stat = "identity", color = maincols[2], fill = maincols[2], width = 0.6) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1, color = "black", size = 1.2) +
  scale_x_discrete(limits = stages, labels = stageNamesNeat) + 
  ylab("Number of people") + xlab("") +  
...
# Save if required
if (saveplots) {
  png(file.path(figFolder, paste("HIVcascade-NSW-", toString(resultsYear), ".png",sep ="")), 
...
for (ii in 1:nrow(stateTable)){
  if (stateTable$stage[ii] == "linked") {
    finalTable$estimate[ii] <- format.data(stateTable$value[ii], places = -1)
  } else {
    finalTable$estimate[ii] <- format.data(stateTable$value[ii], 
                                           stateTable$lower[ii], 
