...
# Removed linked and retained from hivcascade if specified
if (excludeCare) {
...
resultsAll <- filter(hivcascade, population == "all" & 
                       year >= startYear)
resultsAllYear <- filter(resultsAll, year == resultsYear)

# Plot specs
stages <- c("infected", "pldhiv", "linked", 
...
if (pubPlots) {
  # Create an additional stacked bar chart for publications
  ...
  stackPlotBase <- ggplot(data = stackResults, aes(x = year, y = value, 
    ...
    scale_fill_manual(values = asrcols, name = "", labels = plotLabels, 
...
if (savePlots) {
  ...
  ggsave(file.path(figFolder, paste("HIVcascade-all-", 
    ...
    plot = plotBarAll, width =  10, height = 10, units = "cm")
