...
ecdcChart <- paste0(ecdcChart, "-charts.xlsx")
...
numberUndiagnosed <- read_excel(file.path(outputFolder, cascadeName,
                                    ecdcChart),
           sheet = 2, col_names = FALSE, skip = 129) %>%
  select(X1, X10, X11, X12) %>%
  rename(year = X1, undiagnosed = X10, undiagnosed_lower = X11,
         undiagnosed_upper = X12) %>%
  slice(1:length(1980:analysisYear)) %>%
  mutate(year = as.numeric(year))

percentUndiagnosed <- read_excel(file.path(outputFolder, cascadeName,
                                    ecdcChart),
           sheet = 2, col_names = FALSE, skip = 192) %>%
  select(X1, X2, X3, X4) %>%
  rename(year = X1, percent = X2, percent_lower = X3,
         percent_upper = X4) %>%
  slice(1:length(1980:analysisYear))
