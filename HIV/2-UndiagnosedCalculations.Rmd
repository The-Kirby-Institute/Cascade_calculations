...
if (getNewInfections) {
  ecdcChartExclude <- paste0(cascadeFlag, "_Charts", "_exclude", ecdcType)
}
...
if (cascadeFlag == "Otherexp") {
  ...
  if (getNewInfections) {
    # Need excluded file as well
    ecdcFileExclude <- file.path(outputFolder, projectFolder, "ECDC_models",
  ...
  if (getNewInfections) {
    # Need excluded file as well
    ecdcFileExclude <- file.path(outputFolder, projectFolder, "ECDC_models",
      cascadeName, paste0(ecdcResults, "_exclude"), 
      paste0("results/All_Charts", ecdcType))
  }
}
...
finalFile <- file.path(outputFolder, projectFolder, cascadeName,
  ecdcChart)
if (getNewInfections) {
  finalFileExclude <- file.path(outputFolder, projectFolder, cascadeName,
    ecdcChartExclude)
}
...
if (!file.exists(finalFile) || updateECDC) {
  ...
  if (getNewInfections) {
    file.copy(ecdcFileExclude, finalFileExclude, overwrite = TRUE)
  }
  fileFlag <- TRUE
...
if (getNewInfections) {
  # Also grab the new infections estimates
  
  # For all diagnoses
  newInfections <- read_excel(file.path(outputFolder, projectFolder, 
    cascadeName, ecdcChart), sheet = 2, col_names = FALSE, skip = 3) %>%
    select(X__2, X__3, X__4, X__5) %>%
    rename(year = X__2, infections = X__3, infections_lower = X__4,
      infections_upper = X__5) %>%
    slice(1:length(1980:analysisYear)) %>%
    mutate(year = as.numeric(year)) %>%
  ...
  newInfectionsExclude <- read_excel(file.path(outputFolder, projectFolder, 
    cascadeName, ecdcChartExclude), sheet = 2, col_names = FALSE, 
    skip = 3) %>%
    select(X__2, X__3, X__4, X__5) %>%
    rename(year = X__2, infections = X__3, infections_lower = X__4,
      infections_upper = X__5) %>%
