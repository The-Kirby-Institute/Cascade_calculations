...
ecdcChart <- paste0(cascadeFlag, "_Charts.xlsx")
...
if (cascadeFlag == "OtherExp") {
  ...
  ecdcFile <- file.path(outputFolder, "ECDC_models", paste0("All-",
    toString(analysisYear)), ecdcResults, "results/All_Charts.xlsx")
} else {
  ecdcFile <- file.path(outputFolder, "ECDC_models", cascadeName, 
                      ecdcResults, "results/All_Charts.xlsx")
}

finalFile <- file.path(outputFolder, cascadeName, ecdcChart)

fileFlag <- FALSE

if (!file.exists(finalFile) || updateECDC) {
  file.copy(ecdcFile, finalFile)
  fileFlag <- TRUE
...
if (saveResults) {
 # Save cascade estimates
  ...
  # Save plhiv
  saveStringUndiagnosed <- file.path(outputFolder, cascadeName, 
    paste0("plhivEstimates-", toString(analysisYear)))
