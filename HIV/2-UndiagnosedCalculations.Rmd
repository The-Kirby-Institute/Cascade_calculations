...
analysisYear <- 2021
saveResults <- TRUE 
projectFolder <- "Testing" # "ASR_2022"
                            # "HIV_Cascade_Paper"
cascadeFlag <- "All-new" # minus year
...
addExclude <- TRUE
undiagnosedMethod <- "include" # "include", "exclude", "average" 
...
if (addExclude) {
  ...
  mainUndiagnosedExclude <- read_csv(file.path(outputFolder, projectFolder, 
    ...
    select(year, N_Und, N_Und_Alive_p) %>%
    rename(undiagnosed_exclude = N_Und, percent_exclude = N_Und_Alive_p)
  ...
  undiagnosedExclude <- left_join(mainUndiagnosedExclude, 
    ...
    select(year, undiagnosed_exclude, undiagnosed_lower_exclude, 
      undiagnosed_upper_exclude, percent_exclude, percent_lower_exclude,
      percent_upper_exclude)
