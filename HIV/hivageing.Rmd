...
load(file.path(resultsFolder, paste("HIVresults-",
                    toString(analysisYear), "_gender.rda", sep = "")))
...
# By gender
uniqueCasesMale <- filter(hivGender, sex == "male")$propunique *
  filter(hivGender, sex == "male")$totalnotifications
unqiueDiagsMale <- c(uniqueCasesMale[1], diff(uniqueCasesMale))

uniqueCasesFemale <- filter(hivGender, sex == "female")$propunique *
...
absDeathRatesAll <- filter(absDeathRates, sex == "male")
absDeathRatesMale <- filter(absDeathRates, sex == "male")
absDeathRatesFemale <- filter(absDeathRates, sex == "female")
...
for (ii in 2:nyears) {
  ...
  if (yearsVec[ii - 1] <= 2000) {
    absDeaths <- filter(absDeathRatesAll, year == 2000)$deathrate
  } else if (yearsVec[ii - 1] >= 2011) {
    absDeaths <- filter(absDeathRatesAll, year == 2011)$deathrate
  } else {
...
for (ii in 2:nyears) {
  ...
  # Extract abs death rates for this year
  if (yearsVec[ii - 1] <= 2000) {
    absDeaths <- filter(absDeathRatesMale, year == 2000)$deathrate
  } else if (yearsVec[ii - 1] >= 2011) {
    absDeaths <- filter(absDeathRatesMale, year == 2011)$deathrate
  } else {
...
for (ii in 2:nyears) {
  ...
  # Extract abs death rates for this year
  if (yearsVec[ii - 1] <= 2000) {
    absDeaths <- filter(absDeathRatesFemale, year == 2000)$deathrate
  } else if (yearsVec[ii - 1] >= 2011) {
    absDeaths <- filter(absDeathRatesFemale, year == 2011)$deathrate
  } else {
...
print(grid.arrange(barPlotAll, propPlotAll, ncol = 2, widths = c(1, 1.2)))
```
