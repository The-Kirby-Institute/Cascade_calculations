...
analysisYear <- 2023
projectName <-  "HIV_Cascade_Paper" # "ASR_2025"; "HIV_Cascade_Paper"

multipleCascades <- FALSE
artNames <- NULL

# If this is an ASR project already have specific populations
if (multipleCascades && projectName == "ASR_2025") {
  cascadeNames <- c("All", "NonIndigenous", "AusNonIndigenous", "Indigenous", "Male", 
  "Female", "SCA", "SSA", "SEA", "MSM", "MSM_AUS", "MSM_OS", "Hetero",
  "Hetero_Aus", "Hetero_Os", "Pwid")
} else {
...
for (ii in 1:length(cascadeNames)) {
  ...
  if (tolower(artNames[ii]) %in% artSuppression$population) {
    ...
    if (adjustLower) {
      if (nrow(treatment) > 0) {
        ...
        # 
        # Also make sure retained estimate isn't lower than the ART estimate. 
        
        treat <- (treatment %>% filter(stage == "numART"))$value
        ...
        retainedNew$value <- ifelse(
          retainedNew$value < treat[(length(treat) - 
              nrow(retainedNew) + 1):length(treat)], 
          treat[(length(treatLower) - nrow(retainedNew) + 1):length(treat)],
          retainedNew$value)
  ...
  } else if (length(linkagePop) != 0) {
    if (linkagePop %in% linkageART$name) {
  ...
  # Bind cascade and round so CSV is read by excel correctly
  hivCascade <- bind_rows(living, pldhiv, treatment) |>
    mutate(population = tolower(cascadeNames[ii])) |>
    dplyr::select(stage, year, population, everything()) |> 
    mutate_at(c("value", "lower", "upper"), \(x) round(x, digits = 5))
  
  # Save if required
  if (saveResults) {
    write_excel_csv(hivCascade, paste0(cascadeFolder, 
...
if (saveResults && mergeAll) {
  write_excel_csv(hivCascadeAll, file.path(outputFolder, projectName,
