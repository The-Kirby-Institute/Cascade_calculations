...
table(imputeSets$imp$sex) / sum(table(imputeSets$imp$sex)) # imputed

# cob - biggy

# agehiv
densityplot(imputeSets, ~agehiv)

# expgroup
table(imputeData$expgroup, useNA = "ifany")
table(imputeData$expgroup) / sum(table(imputeData$expgroup)) # data
table(imputeSets$imp$expgroup) / sum(table(imputeSets$imp$expgroup)) # imputed
