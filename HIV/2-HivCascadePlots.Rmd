...
figFolder <- file.path(basePath, "output","figures","hiv_cascade") #figFolder <- file.path(basePath,"output", "figures", "hiv_cascade")
...
if (pubPlots) {
  ...
  suppressPlot <- ggplot(data = suppressData, 
                         aes(x = year, y = 100 * suscov)) +
    geom_ribbon(aes(ymin = 100 * suscov_min, ymax = 100 * suscov_max), 
              fill = asrcols[2], alpha = 0.4) + 
    geom_line(color = asrcols[2])  +
    scale_x_continuous(breaks = seq(startYear, resultsYear, by = 3)) +
    ylim(c(0, 100)) +
  ...
  # Save plots
  ggsave(file.path(figFolder, paste("Suppression_coverage_", 
      toString(startYear), "-", toString(resultsYear), 
      saveFormat, sep = "")),
...
if (pubPlots) {
  ...
  linkPlot <- ggplot(data = filter(hivResults, yeardiagnosis >= 1984),
                     aes(x = yeardiagnosis, y = 100 * proplinked)) + 
...
# Reformat names
colnames(finalTable) <- c("State", stageNames[c(1:2, 4:6)])
...
```{r Male and female cascades, echo = FALSE, messages = FALSE, include = FALSE}
...
```{r createtable5, echo = FALSE, messages = FALSE, include = FALSE}
...
```{r insertplot6, echo = FALSE, messages = FALSE, warning = FALSE, results = "hide", fig.width= 5.5, fig.height=7}
...
```{r inserttable5, echo = FALSE, messages = FALSE, warning = FALSE, results = "asis"}
