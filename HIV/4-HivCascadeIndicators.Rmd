...
```{r Options}
projectName <- "Undiagnosed_Metrics_Test"   # "ASR_2022" 
# "HIV_Cascade_Paper"
...
# List for YDF paper
resultsYear <- 2019
cascadeList <- "MSM-Aus-2019" #(Include year) "All-2021"
...
# overseas)to get range in deaths and the estimated annual 
# number of diagnoses
...
# Read in files with diagnoses excluding previously diagnosed overseas as 
# well 
...
IndicatorResults <- function(cascadeName) {
  ...
  pldhivAll <- read_csv(file.path(project, cascadeName, diagnosesFile),
    ...
    select(year, diagnoses, duplicates, deaths)
  ...
  if (diagsCalculation == "include") {
    ...
    if (adjustDiagnoses) {
      ...
      diagnosesLower <- pldhivLower %>% 
        mutate(adjusted = diagnoses - duplicates) %>% 
        select(year, adjusted) %>% 
        rename(lower = adjusted)
      diagnosesUpper <- pldhivUpper %>% 
        mutate(adjusted = diagnoses - duplicates) %>% 
        select(year, adjusted) %>% 
        rename(upper = adjusted)
    ...
    diagnoses <- diagnosesInclude %>%
      ...
      mutate(diagnoses = (diagnoses_include + diagnoses_exclude) / 2,
        lower = (lower_include + lower_exclude) / 2, 
        upper = (upper_include + upper_exclude) / 2) %>%
      select(year, diagnoses, lower, upper)
