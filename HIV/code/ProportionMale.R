...
ProportionMale <- function(hivSet, analysisYear, doAge, 
  ...
  if (doAge) {
    ...
    if (targetGender == "all" && nrow(hivSet) != 0) {
      ...
      hivSetGenderAge <- hivSetGenderAge %>%
        mutate(other = Reduce("+", 
          dplyr::select(., -yeardiagnosis, -agebin, -male))) %>%
        rename(year = yeardiagnosis) %>%
          dplyr::select(year, agebin, male, other)
    ...
    if (targetGender == "all" && nrow(hivSet) != 0) {
      ...
      hivSetGender <- hivSetGender %>%
        mutate(other = Reduce("+", 
          dplyr::select(., -yeardiagnosis, -male))) %>%
        rename(year = yeardiagnosis) %>%
          dplyr::select(year, male, other)
