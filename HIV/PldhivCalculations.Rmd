...
if (targetCob != "all") {            
  adjustDiags <- hivSet %>% 
    ...
    mutate(prop_known = subset / cob_known) %>%
    mutate(prop_known = ifelse(is.nan(prop_known),0, prop_known)) %>%
    mutate(adjusted_subset = round(subset  + cob_unknown * prop_known)) %>%
    rename(year = yeardiagnosis)
  
  subsetDiags <- adjustDiags %>% 
    select(year, adjusted_subset) %>%
    rename(subset = adjusted_subset)
...
hivResults$notifications <- subsetDiags$subset
...
cumDiagnoses <- round(hivResults$uniquecases)
