...
subsetRates <- GetAdjustments(hivBase, hivAdjustments, hivInterstate, 
                              targetAge, targetExposure, targetCob,
...
pldhivAll <- LivingDiagnosed(adjustDiags$included,
                             ...
                             subsetRates$mrate,
                             subsetRates$propstay, 
                             arrivals = subsetRates$arriverate,
                             departs = subsetRates$departrate,
...
if (ecdcData) {
  ...
  # TODO: excluding previously diagnosed overseas 
  hivExpCum <- hivSet %>% 
  ...
  expEmig[, 2:5] <- expEmig[, 2:5] * 
    matrix(rep(emigrants$total, 4), ncol = 4)
