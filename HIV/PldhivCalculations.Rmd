...
agedSubset <- bind_cols(knownDiags, as.data.frame(propAge))

# Number in each CD4 count---------------------------------------------
# This is for ECDC where we have CD4 by exposure

cd4All <- function(hivData, cd4binGroup, 
...
cd4Exposure <- function(hivData, cd4binGroup,
  ...
  cd4ExpBin <- cd4DiagsExp %>%
    ...
    mutate(total = hetero+msm+otherexp+pwid+unknown) %>%
    mutate(hetero = hetero / (1 - unknown / total),
           msm = msm / (1 - unknown / total),
           pwid = pwid / (1 - unknown / total),
           otherexp = otherexp / (1 - unknown / total)) %>%
    dplyr::select(-cd4bin, -total, -unknown) %>%
    rename(year = yeardiagnosis)
...
# Diagnosis type----------------------------------------------------------
diagType <- hivData %>%
  group_by(yeardiagnosis, typediagnosis, expgroup) %>%
  summarise(diags = n()) %>%
  ungroup() %>%
  spread(expgroup, diags)
...
%>%
  dplyr::select(-yeardiagnosis)
diagType[is.na(diagType)] <- 0
propType <- t(apply(diagType, 1, 
                 function(row) row/sum(row)))

knownDiags <- bind_cols(knownDiags, diagType)
