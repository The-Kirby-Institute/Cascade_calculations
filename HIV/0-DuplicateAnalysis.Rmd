...
if (recreateResults) {
  ...
  femaleHiv <- SubHivSet(hivSetAll, "all", "female", "all", "all", "all",
    "all", "all")[[1]]
  msmHiv <- SubHivSet(hivSetAll, "all", "male", "msm", "all", "all",
    "all", "all")[[1]]
...
# Figure 5 ---------------------------------------------------------------
# Compare MSM and hetero males
propMaleData <- filter(duplicateResults, 
  population %in% c("all", "male", "msm", "nonnswmale"), replace == "2022")

maleGroups <- c("nonnswmale", "msm", "male", "all")
maleNames <- c("nonnswmale" = "Heterosexual males",
...
propPlotMale <- ggplot(data = propMaleData, aes(x = year, y = cumunique,
  colour = population)) + 
  geom_line() +
  scale_colour_manual(name = "Population", 
    breaks = maleGroups,
    limits = maleGroups,
    labels = sexNames[maleGroups],
    values = sexCols[maleGroups]) + 
  coord_cartesian(xlim = c(1986, analysisYear), ylim = c(0.75,1)) +
  scale_y_continuous(label = percent) +
  labs(y = "Cumulative proportion", x = "Year",
    title = "Proportion unique by male group") + 
  plotOpts

# Figure caption 
figs("figure5", "Cumulative proportion unique for male groups compared to 
rest of Australia.")
...
```{r Figure 5, include = TRUE, fig.width = 6, fig.height = 3}
figure5
```
...
## Annual duplicates and proportion unique

As stated previously when we estimated the number and proportion of duplciates 
annually we sometimes get a proportion > 1 and a negative number of duplicates. 
This is an artefact of the statistical method and calculation because we use the cumulative number to estimate annual duplicate numbers. Below is a table illustrating this using the national results from the last 10 years.
