...
source(file.path(HIVcode, "ResultsFunctions.R"), echo=TRUE)
...
for (ii in steps) {
  ...
  if (class(model) == "segmented") {
    ...
    tempTable <- tibble(indicator = rep(indicatorNames[ii], nrows),
      breakpoints = c(bps[, 1],0),
      breakpoints_low = c(bps[, 2],0),
      breakpoints_up = c(bps[, 3],0),
      rate_ratio = unname(slopes[, 1]), 
      rate_ratio_low =  unname(slopes[, 2]),
      rate_ratio_up =  unname(slopes[, 3])) 
  } else {
    nrows <- 1
    
    tempTable <- tibble(indicator = indicatorNames[ii],
      breakpoints = 0,
      breakpoints_low = 0,
      breakpoints_up = 0,
      rate_ratio = unname(coef(model)[2]), 
      rate_ratio_low = unname(confint(model)[2, "2.5 %"]),
      rate_ratio_up = unname(confint(model)[2, "97.5 %"])) 
  } 
  ...
  tempTable$valueStart <- rep(tempData$value[1], nrows)
  tempTable$valueStart_low <- rep(tempData$lower[1], nrows)
  tempTable$valueStart_up <- rep(tempData$lower[1], nrows)
  
  tempTable$valueEnd <- rep(tempData$value[nyears], nrows)
  tempTable$valueEnd_low <- rep(tempData$lower[nyears], nrows)
  tempTable$valueEnd_up <- rep(tempData$lower[nyears], nrows)
