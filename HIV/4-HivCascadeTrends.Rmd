...
trendDiagsExclude <- glm(diags ~ year, data = diagsDataExclude, 
  family = "poisson")
coeff <- exp(coef(trendDiagsExclude)[2])
p_value <- summary(trendDiagsExclude)$coefficients["year", "Pr(>|z|)"]
interval <- exp(confint(trendDiagsExclude, "year", type = "Wald"))

trendDiagsInclude <- glm(diags ~ year, data = diagsDataInclude, 
  family = "poisson")
coeff <- exp(coef(trendDiagsInclude)[2])
p_value <- summary(trendDiagsInclude)$coefficients["year", "Pr(>|z|)"]
interval <- exp(confint(trendDiagsInclude, "year", type = "Wald"))
...
diagsPlot <- ggplot(data = diagsDataExclude, aes(x = year)) +
  ...
  geom_line(aes(y = predict(trendDiagsInclude, type = "response")), 
    colour = "black") +
  scale_colour_manual(name = "Notifications", values = c("red", "blue"),
...
trendModels[["infections"]] <- trendInfects
