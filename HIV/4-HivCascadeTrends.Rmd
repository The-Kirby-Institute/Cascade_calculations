...
indicatorNames <- c("infected" = "Living with HIV",
  ...
  "propdiagnosed" = "Proportion undiagnosed",
  "proptreated" =  "Proportion diagnosed treated",
  "propsuppressed" = "Proportion treated suppressed",
  "propoverall" = "Proportion suppressed overall",
...
Segment <- function(indicator, initialbp, ymax, yLabel, minimum = NA, 
  maximum = NA, offset = 1, 
  min_gap = 3, yPercent = TRUE, save = FALSE) {
  ...
  bestIndex <- filter(eligible, 
    number == which.min(eligible$bic))$number
...
# Figure captions
captions[["cascadeFull"]] <- "HIV notifications in Australia and Australia's HIV cascade to the end of 2020. A) Annual notifications data . B) HIV cascade at end of 2020 including estimates for retained in care. Horizontal lines show the UNAIDS 95-95-95 targets. C) Estimate numbers and percentages of people living with HIV in each step of the HIV cascade over 2004-2020.  "
...
indicatorsFullPlot <- ggdraw() +
  ...
  draw_plot_label(c("A", "B", "C", "D", "E"), 
    c(0, 0.5, 0, 0.5, 0.25), c(0.95, 0.95, 0.633, 0.633, 0.316), 
    size = 12)
...
# Need to overwrite proportion estimates so they are not rounded
formatTable$valueStart[25:40]  <- FormatData(resultsTable$valueStart[25:40],
  resultsTable$valueStart_low[25:40], resultsTable$valueStart_up[25:40], 
  places = 2, decimal = "mid")
formatTable$valueEnd[25:40] <- FormatData(resultsTable$valueEnd[25:40],
  resultsTable$valueEnd_low[25:40], resultsTable$valueEnd_up[25:40], 
  places = 2, decimal = "mid")
