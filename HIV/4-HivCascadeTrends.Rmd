...
cascadeName <- "All-2023" # All-2023, Female-2023, Male-2023
cascadePop <- "all" # all, female, male

# Limit populations for the moment
if (!(cascadePop %in% c("all", "female", "male"))) {
  stop("Script not setup for other populations")
}
...
allIndicators <- c("infected", "pldhiv", "retained", "numART", "suppressed",
  ...
  "diagnoses_include", "diagnoses_os", "infections", "propdiagnosed", "proptreated",
  "propsuppressed", "propoverall", "ydf", "cdr", "ipr", "imr")

indicatorNames <- c("infected" = "People living with HIV",
  "pldhiv" = "Diagnosed",
  ...
  "diagnoses_os" = "Diagnoses overseas",
...
indicatorNamesNeat <- c("infected" = "Number living with HIV",
  ...
  "diagnoses_os" = "Annual notifications\n(previous diagnosis OS)", 
  "infections" = "Annual new infections\n ",
  "propdiagnosed" = "Percentage of people\nliving with HIV diagnosed", 
...
# Filter out cascadePop
if (cascadePop != "all") {
  hivSet <- hivSet |>
    filter(sex == cascadePop)
}
...
independent covariate for the main cascade steps and indicators over 2004-2023. 
...
diagsPlot <- ggplot(data = diagsDataExclude, aes(x = year)) +
  ...
  scale_y_continuous(labels = comma, limits = c(0, diagsMax)) +
...
trendData <- bind_rows(trendData,
  ...
  diagsDataInclude %>% 
    ...
    dplyr::select(year, stage, value = diags, lower, upper), 
  diagsDataOS %>% 
    mutate(stage = "diagnoses_os", lower = NA, upper = NA) %>% 
    dplyr::select(year, stage, value = diags, lower, upper), 
  newInfects %>% 
...
# legend <- GetLegend(diagsPropPlot)
...
# legend <- GetLegend(trendPlots$diagnoses_include)
...
combinedIndicatorPlot <- plot_grid(
  ...
  trendPlots$diagnoses_os + theme(legend.position="none"),
...
if (savePlots) {
  ggsave(file.path(figFolder,
    paste("HIVcascade_Indicators_Trends_",
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = combinedIndicatorPlot, width = 16, height = 32, units = "cm")
}
...
diagnoses_osts <- tots("diagnoses_os")
...
data <- cbind(infectedts, pldhivts, numARTts, suppressedts, undiagnosedts,
  ...
  diagnoses_osts, infectionsts, propdiagnosedts, proptreatedts, propsuppressedts, propoverallts,
  ydfts, cdrts, iprts, imrts)
colnames(data) <- c("infected", "pldhiv", "numART", "suppressed",
  "undiagnosed","noART", "unsuppressed", "diagnoses_exclude", "diagnoses_os", 
...
autoplot(cpt.mean(diagnoses_osts))
...
# Sometimes the indicators produce errors so I have commented them out and 
# replaced with a NULL as this is just for exploration
...
BPindicator <- function(indicator, tsdata) {
  ...
  plot(tsdata)
  lines(bpIndicator)
  # CIs might not be able to be calculated so ignore and return a NULL
  res <- try(lines(confint(bpIndicator)), silent = TRUE) 
...
bpDiagnoses_os <- BPindicator("diagnoses_os", diagnoses_osts)
...
# can't be calculated (returns NA). Seem to get negative lower ranges sometimes 
# so just replace with first year as just for exloration not analysis
...
for (ii in 1:length(modelNames)) {
  ...
  if (!is.null(tempbp)) {
    ...
    tempDf$break1ci <- c(paste0("(", 
      years[max(confint(tempbp, breaks = 1)$confint[1],1)],
      ", ",
    ...
    tempDf$break2ci <- c(paste0("(", 
      years[max(confint(tempbp, breaks = 2)$confint[1, 1], 1)], 
      ", ",
    ...
    tempDf$break3ci <- c(paste0("(", 
      years[max(confint(tempbp, breaks = 3)$confint[1, 1], 1)], 
      ", ",
...
Segment <- function(indicator, initialbp, ymax, yLabel, minimum = NA, 
  ...
  if (save) {
    ...
    legend1 <- GetLegend(segPlotAll)
    legend2 <- GetLegend(segPlotBest + theme(legend.position = "right"))
...
ymax <- c("infected" = 40000,
  ...
  "noART" = 600, 
  "unsuppressed" = 450,
  "diagnoses_exclude" = 150,
  "diagnoses_include" = 250,
  "diagnoses_os" = 200,
  "infections" = 120, 
  "propdiagnosed" = 1.0, 
  "proptreated" = 1.0,       
  ...
  "noART" = 6000, 
  "unsuppressed" = 3000,
  "diagnoses_exclude" = 1000,
  "diagnoses_include" = 1200,
  "diagnoses_os" = 500,
  "infections" = 1000, 
  "propdiagnosed" = 1.0, 
  "proptreated" = 1.0,       
  ...
  stor("Unknwon cascade population")
}
...
plotCascadeAll <- PlotCascade(latestCascade, retained = TRUE, ymax = cascadeMax,
  plotcolours = asrcols,  
  steplabels = c("Living\nwith HIV", "Diagnosed", "Retained\nin care",
    "Receiving\ntreatment", "Suppressed\n(VL < 200)"), 
  percentages = TRUE,
  targetlines = "95", targetlabels = FALSE) + 
  scale_y_continuous(breaks = seq(0, cascadeMax, 
    by = 10^floor(log10(cascadeMax))),
    labels =  comma,
    limits = c(0, cascadeMax)) +
...
# Figure captions
captions[["cascadeFull"]] <- "HIV notifications in Australia and Australia's HIV cascade to the end of 2023. A) Annual notifications data. B) HIV cascade at end of 2023 including estimates for retained in care (note lower range of Diagnosed and Retained in care adjusted to equal lower range of number Receiving treatment). Horizontal lines show the UNAIDS 95-95-95 targets. C) Estimate numbers and percentages of people with HIV in each step of the HIV cascade over 2004-2023.  "

# Cascade trends ---------------------------------------------------------------
legend <- GetLegend(segResults[["infected"]]$plotBest)
...
captions[["stepTrends"]] <- "Estimates and trends for each HIV cascade step over 2004-2023. Red lines and shading show the estimated values and range from the HIV cascade calculations. Black lines show the piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points. "

# Cascade gaps -----------------------------------------------------------------
legend <- GetLegend(segResults[["undiagnosed"]]$plotBest)
...
captions[["cascadeGaps"]] <- "Trends in the estimated gaps of the HIV cascade over 2004-2023. Red lines show the estimated values from the HIV cascade calculations. Black lines show the best fitting piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points."

# Remaining cascade indicators --------------------------------------------------- 
legend <- GetLegend(segResults[["diagnoses_include"]]$plotBest)
...
indicatorsFullPlot <-  ggdraw() +
  ...
  draw_plot(
    plot_grid(RmLegened(segResults[["diagnoses_include"]]$plotBest),
      ...
      RmLegened(segResults[["diagnoses_os"]]$plotBest),
      ...
      labels = c("A", "B", "C", "D", "E", "F", "G", "H")), 0, 0, 1, 0.95)

captions[["cascadeIndicators"]] <- "Trends in cascade related indicators over 2004-2023. Red lines and shading show the estimated values and range from the HIV cascade calculations. Black lines show the best fitting piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points."
...
if (savePlots) {
  ggsave(file.path(figFolder,
    paste("HIVcascade_Full_Plot_",
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = cascadeFullPlot, width = 25, height = 18, units = "cm")
...
if (runtests) {
  ...
  legend <- GetLegend(segTestPlot[[1]])
...
if (runtests) {
  ...
  # Create combined plot
  legend <- GetLegend(bootstrapPlots[[1]])
