...
LoadLibrary(aod)
step <- "noART" #diagnoses, undiagnosed, noART, unsuppressed
...
modelTable <- data.frame(indicator = unname(modelString[step]))
modelTable$parameter <- coef(overallModel)["year"]
modelTable$param_ci_lower <-confint(overallModel, "year", type = "Wald")[1]
modelTable$param_ci_upper <-confint(overallModel, "year", type = "Wald")[2]
modelTable$pvalue <- summary(overallModel)$coefficients["year",
    "Pr(>|z|)"]
modelTable$pseudoR2 <- rsq(overallModel, type = "kl")
modelTable$bic <- BIC(overallModel)

modelTable$cpt <-  segModel$psi[, "Est."]
modelTable$cpt_ci_lower <- segCI(segModel$psi[, "Est."],
  segModel$psi[, "St.Err"])[1]
modelTable$cpt_ci_upper <- segCI(segModel$psi[, "Est."],
  segModel$psi[, "St.Err"])[2]
modelTable$slope1 <- slope(segModel)$year["slope1", "Est."]
modelTable$slope1_ci_l <- slope(segModel)$year["slope1", "CI(95%).l"]
modelTable$slope1_ci_u <- slope(segModel)$year["slope1", "CI(95%).u"]
modelTable$slope2 <- slope(segModel)$year["slope2", "Est."]
modelTable$slope2_ci_l <- slope(segModel)$year["slope2", "CI(95%).l"]
modelTable$slope2_ci_u <- slope(segModel)$year["slope2", "CI(95%).u"]
modelTable$slopes_p <- 2*pnorm(-abs(wstat)) # test stat from nomal
modelTable$seg_pseudoR2 <- rsq(segModel, type = "kl")
modelTable$seg_bic <- BIC(segModel)
modelTable$pchow <- pchow
