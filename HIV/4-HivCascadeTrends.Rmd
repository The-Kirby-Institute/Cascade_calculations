...
minYear <- 18      # how long ago do we plot things
...
advanced trend analyses for the cascade steps was completed by Hamish McMannus
...
We aimed to produce a number of diagnostics of the poisson fit. We considered
...
regression model, Journal of Econometrics, 77 (1997) 329-342. However, this
package does not seem to work in the latest versions of R so I decided to 
drop it. 
...
diagsPlot <- ggplot(data = diagsDataExclude, aes(x = year)) +
  ...
  scale_x_continuous(breaks = seq(startYear, resultsYear),
    labels =  EveryNth(seq(startYear, resultsYear), xTicks, inverse = TRUE),
...
trendData <- bind_rows(trendData, tibble(year = year,
  stage = "ydfoffest", 
  value = hivCascadeIndicators$undiag + 
    filter(trendData, stage == "diagnoses_include")$value, 
  lower = hivCascadeIndicators$undiag_lower + 
    filter(trendData, stage == "diagnoses_include")$value, 
  upper = hivCascadeIndicators$undiag_upper+ 
    filter(trendData, stage == "diagnoses_include")$value))
trendData <- bind_rows(trendData, tibble(year = year,
  stage = "iproffset",
...
ydfModel <- glm.nb(diagnoses_include ~ year + offset(log(ydfoffest)), 
  data = expTrendData)
cdrModel  <- glm.nb(diagnoses_include ~ year + offset(log(infections)),
  data = expTrendData)
...
cdrPropPlot <- plotProp(cdr, cdrModel, 
  expTrendData$cdroffset, yLabels["cdr"], 
  trend = plotTrends) +
  # Fix up y axis - will get warnings about replacement
...
propTrendsIndicators <- tibble(stage = c("ydf", "cdr", "ipr", "imr"),
  ...
  model = "negative-binomial-offset")
...
bpPropoverall <- BPindicator("propoverall", propoverallts) # get error with confidence intervals
...
bpmodels <- list(bpInfected, bpPldhiv, bpNumART, bpSuppressed, bpUndiagnosed,
  ...
  bpPropoverall, bpYdf, bpCdr, bpIpr, bpImr)
...
Segment <- function(indicator, initialbp, ymax, yLabel, minimum = NA, 
  ...
  if (allow_end_gap) {
    # Allow gap at end to be less than min_gap. Means can have a change point 
    # after 2019 which is justifiable I think due to COVID. 
    modelBreakpointGaps <- tibble(model = c("pmodel", "segModel1", 
      "segModel2", "segModel3","segModel4"),
      minimum_gap = c(resultsYear - startYear, 
        min(diff(c(startYear, unname(segModel1$psi[, "Est."])))), 
        min(diff(c(startYear, unname(segModel2$psi[, "Est."])))),
        min(diff(c(startYear, unname(segModel3$psi[, "Est."])))), 
        min(diff(c(startYear, unname(segModel4$psi[, "Est."])))))) 
  } else {
    modelBreakpointGaps <- tibble(model = c("pmodel", "segModel1", 
      "segModel2", "segModel3","segModel4"),
      minimum_gap = c(resultsYear - startYear, 
  ...
  segPlotAll <- ggplot(data = tempData, 
    ...
    geom_text(x = 2012, y = 0.15 * ymax, 
      label = sprintf("BIC values\nOverall %s; One CP %s; Two CPs %s; Three CPs %s; Four CPs %s.", 
        bicStr[1], bicStr[2], bicStr[3], bicStr[4], bicStr[5]), size = 3) 
  
  if (allow_end_gap) {
    segPlotAll <- segPlotAll +
      geom_text(x = 2012, y = 0.05 * ymax, 
        label = sprintf("Minimum gaps (excluding end)\nOverall %s; One CP %s; Two CPs %s; Three CPs %s; Four CPs %s.", 
          gapStr[1], gapStr[2], gapStr[3], gapStr[4], gapStr[5]), size = 3)
  } else {
    segPlotAll <- segPlotAll +
      geom_text(x = 2012, y = 0.05 * ymax, 
  ...
  modelBreakpointGaps$number <- 1:5
  ...
  if (save) {
    ...
    # segPlotCombine <- plot_grid(segPlotAll, 
    #   segPlotBest + theme(legend.position = "right"),
    #   ncol = 1, labels = c("A", "B"))
...
for (ii in steps) {
  ...
  segResults[[ii]] <- Segment(ii, initialbp, ymax[ii], 
    ...
    # min_gap = 3, allow_end_gap = TRUE,
    yLabels[ii], save = TRUE, offset = offsets[[ii]], 
    yPercent = yPercent[ii])
}
...
# This chunk is used to estimate indicator values to 2030. 

# Cascade steps
infectedPredict <- exp(predict(segResults$infected$models[[segResults$infected$bestIndex]], 
  newdata = data.frame(year = 2020:2030)))
...
propdiagnosed95 <- exp(predict(segResults$propdiagnosed$models[[segResults$propdiagnosed$bestIndex]],
  newdata = data.frame(year = 2020:2030, infected = infectedPredict)))/infectedPredict
proptreated95 <- exp(predict(segResults$proptreated$models[[segResults$proptreated$bestIndex]],
  newdata = data.frame(year = 2020:2030, pldhiv = diagnosedPredict)))/diagnosedPredict
propsuppressed95 <- exp(predict(segResults$propsuppressed$models[[segResults$propsuppressed$bestIndex]],
  newdata = data.frame(year = 2020:2030, numART = numARTPredict)))/numARTPredict
...
plotCascadeAll <- PlotCascade(latestCascade, retained = TRUE, ymax = 35000,
  ...
  percentages = TRUE,
  targetlines = "90", targetlabels = FALSE) + 
...
cascadeTrendPlot <- ggdraw() + 
  ...
  draw_plot(RmLegened(segResults[["infected"]]$plotBest), 
    0, 0.475, 0.5, 0.475) +
  draw_plot(RmLegened(segResults[["pldhiv"]]$plotBest),
    0.5, 0.475, 0.5, 0.475) +
  draw_plot(RmLegened(segResults[["numART"]]$plotBest),
    0, 0, 0.5, 0.475) +
  draw_plot(RmLegened(segResults[["suppressed"]]$plotBest),
    0.5, 0, 0.5, 0.475) +
  draw_plot_label(c("A", "B", "C", "D"), 
    c(0, 0.5, 0, 0.5), c(0.95, 0.95, 0.475, 0.475), 
    size = 12)
captions[["stepTrends"]] <- "Estimates and trends for each HIV cascade step over 2004-2021. Red lines and shading show the estimated values and range from the HIV cascade calculations. Black lines show the piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points. "
...
gapsFullPlot <- ggdraw() +
  draw_plot(legend, 0, 0.95, 1, 0.05) + 
  draw_plot(RmLegened(segResults[["propdiagnosed"]]$plotBest),
    0.5, 0.633, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["undiagnosed"]]$plotBest), 
    0, 0.633, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["proptreated"]]$plotBest),
    0.5, 0.316, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["noART"]]$plotBest),
    0, 0.316, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["propsuppressed"]]$plotBest),
    0.5, 0, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["unsuppressed"]]$plotBest),
    0, 0, 0.5, 0.316) +
  draw_plot_label(c("A", "B", "C", "D", "E", "F"), 
    c(0, 0.5, 0, 0.5, 0, 0.5), c(0.95, 0.95, 0.633, 0.633, 0.316, 0.316), 
    size = 12)

captions[["cascadeGaps"]] <- "Trends in the estimated gaps in the HIV cascade. Red lines show the estimated values from the HIV cascade calculations. Black lines show the best fitting piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points."
...
indicatorsFullPlot <- ggdraw() +
  draw_plot(legend, 0, 0.95, 1, 0.05) + 
  draw_plot(RmLegened(segResults[["diagnoses_include"]]$plotBest),
    0, 0.633, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["diagnoses_exclude"]]$plotBest),
    0.5, 0.633, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["infections"]]$plotBest), 
    0, 0.316, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["ydf"]]$plotBest),
    0.5, 0.316, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["cdr"]]$plotBest),
    0, 0, 0.5, 0.316) +
  draw_plot(RmLegened(segResults[["ipr"]]$plotBest),
    0.5, 0, 0.5, 0.316) +
  draw_plot_label(c("A", "B", "C", "D", "E", "F"), 
    c(0, 0.5, 0, 0.5, 0, 0.5), c(0.95, 0.95, 0.633, 0.633, 0.316, 0.316), 
    size = 12)

captions[["cascadeIndicators"]] <- "Trends in cascade related indicators. Red lines and shading show the estimated values and range from the HIV cascade calculations. Black lines show the best fitting piecewise Poisson model predictions. Vertical dashed lines show the location of detected change points."
...
if (savePlots) {
  ...
  ggsave(file.path(figFolder,
    paste("HIVindicators_Full_Plot_",
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = indicatorsFullPlot, width = 22, height = 25, units = "cm")
...
formatTable$rate_ratios <- FormatData(resultsTable$rate_ratio,
  resultsTable$rate_ratio_low, resultsTable$rate_ratio_up, places = 4,
  decimal = "mid")

if (savePlots) {
  write_csv(resultsTable, file.path(figFolder, 
    paste0("HIVcascade_Summary_Table_", 
      toString(startYear), "-", toString(resultsYear), ".csv")))
  
  write_csv(formatTable, file.path(figFolder, 
