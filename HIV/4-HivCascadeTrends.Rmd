...
minYear <- 14        # how long ago do we plot things
...
if (infectsCalculation == "include") {
  hivNewInfections <- hivNewInfections %>%
    filter(exclude == "no") %>% 
    select(-exclude)
} else if (infectsCalculation == "include") {
  hivNewInfections <- hivNewInfections %>%
    filter(exclude == "yes") %>% 
    select(-exclude)
} else {
  ...
  hivNewInfections <- data_frame(year = infectsInclude$year,
...
# the growing number of notifications in those previoulsy diagnosed 
# overseas. 
diagsData <- hivSet %>%
...
trendInfects <- glm(round(infections) ~ year, data = newInfects, 
  family = "poisson")
coeffInfects <- coef(trendInfects)[2]
p_valueInfects <- summary(trendInfects)$coefficients["year", "Pr(>|z|)"]
intervalInfects <- confint(trendInfects, "year", type = "Wald")

# Diagnostics
pseudoR2Infects <- rsq(trendInfects, type = "kl")

# Infectiosn plot with diagnoses added
infectsPlot <- ggplot(data = newInfects, aes(x = year, 
  ...
  geom_text(x = 2010, y = 300, label = paste("Pseudo-R^2 infections:",
    ...
    size = 3) + 
  geom_text(x = 2010, y = 150, label = paste("Pseudo-R^2 diagnoses:",
    toString(round(rsq(trendDiags, type = "kl"), digits = 4))), 
    size = 3) 
...
trendCascade <- data_frame(stage = character(9), 
  coeff = numeric(9),
  pvalue = numeric(9),
  lower = numeric(9),
  upper = numeric(9),
...
# Figure captions
figs("cascadeTrends", "Trends in each step of the Australian HIV cascade over 2004-2016. The red discs show the best estimates for the cascade with the shading showing the range. The black lines show the estimatesfrom the fitted Poisson models.")
figs("gapsTrends", "Trends in gaps between the steps of the Australian HIV cascade over 2004-2016. The red discs show the best estimates. The black lines show the estimates from the fitted Poisson models.")
...
# Logistic models ---------------------------------------------------------
# Fit logistic curve to each trend 
...
figs("unaidsTargets", "Trends in the UNAIDS 90-90-90 and overall targets over 2004-2016.")
...
# 
# Unsuppressed doesn't appear to have a single chnage point. 
...
# Summary: break points and slopes for diagnoses
summary(segDiags)
segDiags$psi
...
segInfectsPlot <- ggplot(data = newInfects, 
  ...
  scale_colour_manual("", values = c(asrcols[2], "black", "blue"),
    ...
    labels = c("Estimates", "Overall model",
      "Segmented model")) +
  coord_cartesian(xlim = c(2004, 2016), ylim = c(0, infectsmax)) + 
...
# Summary: break points and slopes for infections
summary(segInfects)
segInfects$psi
...
stagesDiffsPlot3 <- ggdraw() +
  draw_plot(combinedPlot2, 0, 0, 1, 1) +
  draw_plot_label(c("A", "B", "C", "D"), c(0, 0.5, 0, 0.5),
    c(0.9, 0.9, 0.45, 0.45), size = 12)
...
Thi section applies some statistical tests for the segmented model to test 
...
# This chunck runs chow and interaction tests for each intermediate
# cascade step. 
LoadLibrary(aod)
