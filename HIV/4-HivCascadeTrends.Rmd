...
Segment <- function(indicator, initialbp, ymax, yLabel, offset = 1, 
  ...
  segPlotAll <- ggplot(data = tempData, 
    ...
    geom_line(aes(y = fitted(pmodel),
      colour = "overall")) +
    geom_line(data = segModelData, aes(y = segvalue1, 
      colour = "segment1")) + 
    geom_line(data = segModelData, aes(y = segvalue2, 
      colour = "segment2")) + 
    geom_line(data = segModelData, aes(y = segvalue3,
      colour = "segment3")) +
    scale_x_continuous(breaks = seq(startYear,                        
      resultsYear, by = 3)) +
    scale_y_continuous(labels = comma) + 
    ...
    geom_text(x = 2012, y = 0.05 * ymax, 
      label = sprintf("BIC values\nOverall %s; One cp %s; Two cp %s; Three cp  %s", 
        bicStr[1], bicStr[2], bicStr[3], bicStr[4]), size = 3)

  segPlotBest <- segPlotAll
  ...
  return(list(models = list(pmodel, segModel1, segModel2, segModel3),
    fitted = segModelData, breakpoints = modelBreakpoints, slopes = modelSlopes, 
