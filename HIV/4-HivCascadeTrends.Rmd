...
set.seed(2653)
fitIndicators <- c("undiagnosed", "noART", "unsuppressed")
...
for (ii in fitIndicators) { 
  ...
  local({
    ...
    # summary(modelFit)
    # ggplot(tempData, aes(year, value)) +
    # geom_point() +
    #   geom_line(aes(y = predict(modelFit, type = "response")))
    
    tempboots <- bootstraps(tempData, times = 10000, apparent = TRUE)
...
# Update group plot
combinedBootsPlot <- grid.arrange(
  bootstrapPlots[[1]] + theme(legend.position = "none"), 
  bootstrapPlots[[2]] + theme(legend.position = "none"),
  bootstrapPlots[[3]] + theme(legend.position = "none"),
  legend,
  ncol = 2, nrow = 2, 
  widths = c(4, 4), heights = c(4,4))

finalBootsPlot <- ggdraw() +
  draw_plot(combinedBootsPlot, 0, 0, 1, 1) +
  draw_plot_label(c("A", "B", "C"), c(0, 0.5, 0),
    c(1, 1, 0.5), size = 12) 
...
if (savePlots) {
  ...
  ggsave(file.path(figFolder,
    paste(plotFile,
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = finalBootsPlot, width = 18, height = 15, units = "cm")
}
