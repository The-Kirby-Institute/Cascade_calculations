...
# Poisson models are exponentiated to represent a rate ratio. 
...
diagsPlot <- ggplot(data = diagsDataExclude, aes(x = year)) +
  ...
  scale_colour_manual(name = "", values = c("red", "blue", "black"),
    breaks = c("include", "exclude", "trend"), 
    labels = c("Including previously diagnosed overseas", 
      "Excluding previously diagnosed overses", "Trends")) +
...
# Infections plot - dropped comparison to diagnoses
infectsPlot <- ggplot(data = newInfects, aes(x = year, 
  ...
  geom_line(aes(y = predict(trendInfects, type = "response"), 
    colour = "infectsTrend")) +
  # geom_line(aes(y = diagsDataExclude$diags, colour ="diags")) +
  # geom_point(aes(y = diagsDataExclude$diags), colour = asrcols[5]) +
  # geom_line(aes(y = predict(trendDiagsExclude, type = "response"), 
  #   colour = "diagsTrend")) +
  coord_cartesian(ylim = c(0, 1500)) +
  expand_limits(y = 0) +
  ...
  scale_colour_manual(name = "", 
    labels = c("Estimated infections", "Trend"), 
...
# Add diagnoses and infecions data
...
for (ii in plotStages) {
  ...
  local({
    ...
    # Add trend line (and pseudo-R^2/BIC) - note will get warnings
    # for stages where lower and upper are NA
...
# Create a group plots with trend lines
...
save(trendData, diagsData, newInfects, 
  file = file.path(resultsFolder, cascadeName, "CascadeTrendData.Rda"))
save(trendCascade, trendModels, trendCascadeProp, 
