...
trendData <- bind_rows(trendData, tibble(year = year,
  ...
  value = hivCascadeIndicators$undiag + 
    filter(trendData, stage == "diagnoses_exclude")$value, 
...
ydfModel <- glm.nb(diagnoses_exclude ~ year + offset(log(ydfoffest)), 
  data = expTrendData)
cdrModel  <- glm.nb(diagnoses_exclude ~ year + offset(log(infections)),
  data = expTrendData)
...
captions[["cascadeGaps"]] <- "Trends in the estimated gaps of the HIV cascade over 2004-2021. Red lines show the estimated values from the HIV cascade calculations. Black lines show the best fitting piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points."
...
captions[["cascadeIndicators"]] <- "Trends in cascade related indicators over 2004-2021. Red lines and shading show the estimated values and range from the HIV cascade calculations. Black lines show the best fitting piecewise negative binomial model predictions. Vertical dashed lines show the location of detected change points."
...
if (savePlots) {
  ...
  ggsave(file.path(figFolder,
    paste("HIVindicators_Full_Plot_",
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = indicatorsFullPlot, width = 22, height = 32, units = "cm")
...
decimalMark <- "low" # Should be "mid" but produces error!
