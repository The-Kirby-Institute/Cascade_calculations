...
# Trends ----------------------------------------------------
steps <- c(unique(trendData$stage), "infections")
...
diagsExpModel <- glm(pldhiv ~ year, offset = log(expTrendData$infected), 
  data = expTrendData, family = "poisson")
treatedExpModel  <- glm(numART ~ year, offset = log(expTrendData$pldhiv), 
  data = expTrendData, family = "poisson")
suppressedExpModel  <- glm(suppressed ~ year, offset = log(expTrendData$numART), 
  data = expTrendData, family = "poisson")
overallExpModel  <- glm(suppressed ~ year, offset = log(expTrendData$infected), 
  data = expTrendData, family = "poisson")

trendModelsProp[["diagnosedProp"]] <- diagsExpModel
trendModelsProp[["treatedProp"]] <- treatedExpModel
trendModelsProp[["suppressedProp"]] <- suppressedExpModel
trendModelsProp[["overallProp"]] <- overallExpModel
...
trendPlotsProp[["diagnosedProp"]] <- diagsPropPlot
trendPlotsProp[["treatedProp"]] <- treatedPropPlot
trendPlotsProp[["suppressedProp"]] <- suppressedPropPlot
trendPlotsProp[["overallProp"]] <- overallPropPlot
...
# Models - using denominator as an offset - explicitly add location of of 
# offset so segmented fits work: https://stackoverflow.com/questions/19034403/r-package-segmented-with-an-offset-term
ydfModel <- glm(diagnoses_include ~ year, offset = log(expTrendData$ydfoffest), 
  data = expTrendData, family = "poisson")
cdrModel  <- glm(diagnoses_include ~ year, offset = log(expTrendData$infections),
  data = expTrendData, family = "poisson")
iprModel  <- glm(infections ~ year, offset = log(expTrendData$infected), 
  data = expTrendData, family = "poisson")
...
save(trendData,
  file = file.path(resultsFolder, cascadeName, "CascadeTrendData.Rda"))
save(trendCascade, trendModels, trendCascadeProp, trendModelsProp, 
  file = file.path(resultsFolder, cascadeName, "CascadeTrendModels.Rda"))
...
```{r Segmented trends}
# This chuck is used to explore the change in trends for each cascade step and 
# gap. Some seem to have a change point. We try to find a change point for 
# each stage using the segmented package and with Poisson models. Note
...
segmentStages <- names(trendModels)
...
# breakpoints() results and visual inspection and to test for sensitivity. 
...
breakPoints <- list("infected" = 2010,
  "plhiv" = 2010,
  "numART" = 2010, 
  "suppressed" = 2010,
  "undiagnosed" = 2010,
  ...
  local({
    ...
    if (savePlots) {
      ...
      ggsave(file.path(figFolder, paste0("HIVcascade-Trends-", ii,
        ...
        toString(resultsYear),
        saveFormat)),
        plot = segPlots[[ii]], width = 12, height = 8, units = "cm")
    }
  })
}
...
save(trendCascade, trendModels, trendCascadeProp, segModels,  
  file = file.path(resultsFolder, cascadeName, "CascadeTrendModels.Rda"))
...
A segmented Poisson model does not necessarily produce a good fit to some of
the gaps estimates and might not be appropriate for describing the change in trends 
seen in the cascade gaps. Also it is difficult to decide which estimates
are most appropriate to use for the gaps trends analysis, particularly for
the noART gap where the change point could be spurious due to changes in 
data source over time. The advantage of the segmented model is that it is
easily interpretable. It is okay for describing retrospective trends but not as 
good for prediction.

In this section we explore a non-linear least squares fitting of a non-linear 
