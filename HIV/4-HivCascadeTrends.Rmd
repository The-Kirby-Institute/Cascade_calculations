...
indicatorNamesNeat <- c("infected" = "Number living with HIV\n ",
  ...
  "undiagnosed" = "Number ling with HIV\nundiagnosed",
  ...
  "propsuppressed" = "Percentage overall\nsuppressed",
...
Segment <- function(indicator, initialbp, ymax, yLabel, minimum = NA, 
  ...
  bestModelData <- select(modelData, c(1, bestIndex + 1)) 
  ...
  segPlotBest <- ggplot(data = tempData, 
    ...
    scale_colour_manual("", values = c(asrcols[2], "black"), 
      ...
      labels = c("Estimates", "Best fit ")) + # adjust alignment
    ylab(yLabel) + xlab("Year") +  
    PlotOptions() 
...
plotCascadeAll <- PlotCascade(latestCascade, retained = TRUE, ymax = 35000,
  ...
  scale_y_continuous(breaks = seq(0, 35000, by = 5000),
    labels =  EveryNth(seq(0, 35000, by = 5000), 2, inverse = TRUE),
    limits = c(0, 35000))

legend <- get_legend(segResults[["infected"]]$plotBest)

cascadeFullPlot <- ggdraw() +
  draw_plot(diagsPlot, 0, 0.6, 0.5, 0.4) +
  draw_plot(plotCascadeAll, 0.5, 0.6, 0.5, 0.4) +
  draw_plot(legend, 0, 0.55, 1, 0.05) + 
  ...
  draw_plot_label(c("A", "B", "C", "D", "E", "F"), 
    c(0, 0.5, 0, 0.5, 0, 0.5), c(1, 1, 0.55, 0.55, 0.275, 0.275), 
    size = 12)
...
if (savePlots) {
  ggsave(file.path(figFolder,
    paste("HIVcascade_Full_Plot_",
      ...
      toString(resultsYear),
      saveFormat, sep = "")),
    plot = cascadeFullPlot, width = 22, height = 25, units = "cm")
