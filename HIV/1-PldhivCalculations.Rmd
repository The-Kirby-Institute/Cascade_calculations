...
targetExposure <- "msm" #msm, hetero, pwid, otherexp (combination)
...
```{r Temporary projections}
# This is a temporary chunk for quickly generating projections. Quickly put 
# together for Laila's delphi project. She wants a projection of the number
# of HIV positive MSM in 2060. 
# 
# To do the projections we simply assume everything in terms of annual 
# diagnoses and rates stays the same as in 2016 for 2017-2060 period. 
# 
# Also done some projections for reductions in diagnoses due to PrEP. 
# 
# Issues: haven't really accounted for aging properly which is probably 
# necessary 

# Set final year for porjections
projectYear <- 2060
...
# Vectors for initial projections
diagnoses <- ProjVec(adjustDiags$adjusted_included, nprojYears)
annUnique <- c(subsetRates$annunique, rep(1, nprojYears))
...
# Alternative diagnoses vectors showing 35%, 50%, and 100% decrease in 
# diagnoses due to PrEP
diagnoses0 <- c(adjustDiags$adjusted_included, rep(0, nprojYears))
diagnoses35 <- c(adjustDiags$adjusted_included, 
  0.65 * rep(End(adjustDiags$adjusted_included), nprojYears))
diagnoses50 <- c(adjustDiags$adjusted_included, 
  0.5 * rep(End(adjustDiags$adjusted_included), nprojYears))
...
# Future projections for alternative diagnoses scenarios
pldhivAllFuture35 <- LivingDiagnosed(diagnoses35,
      annUnique, 
      deathrate,
      osrate,
      propStay) %>%
...
pldhivAllFuture50 <- LivingDiagnosed(diagnoses50,
      annUnique, 
      deathrate,
      osrate,
      propStay) %>%
...
# No more diagnoses
pldhivAllFuture0 <- LivingDiagnosed(diagnoses0,
      annUnique, 
      deathrate,
      osrate,
      propStay) %>%
