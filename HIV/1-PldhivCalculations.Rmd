...
projectOutput <-"GBM_State_Age" # "." for main output folder 
...
cascadeName <- "NSW-2017-All-Test" # (single)
...
targetState <- rep("nsw", numCascades) # nsw, sa, nt, qld, vic, wa, act
  # tas (combination)
targetLocalRegion <- rep("all", numCascades) # SA3 level region within 
  # Australia. Note if targetLocalRegion != "all" then targetState is 
  # changed to state of targetLocalRegion.
...
ecdcData <- rep(FALSE, numCascades)
...
# TODO: check lengths of all the variables and options and make sure they
# equal numCascades. Do all overall error checking here or move into a
# separate chunk. 
...
### Function for calculations begins around here

```{r Error and option checking}
# Error checking and changes in options based on cascade parameters for 
# specific cascade
...
# Change target state to match targetLocal Region
...
# Specifications for aged cascades-----------------------------------------
if (doAge) {
...
if (interState) {
  ...
  interRegionRates <- GetInterRegion(analysisYear, cleanNom, 
    absInterstate, NULL, "all", "all", targetState[1],
    targetLocalRegion[1],
    assumeAdult = TRUE)
