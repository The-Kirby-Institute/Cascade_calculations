...
doUnique <- TRUE
yearUnique <- 2017 #2100 to turn off
saveUnique <- FALSE
...
if (interState) {
  ...
  if (doAge) {
    ...
    pldhivOverall <- LivingDiagnosed(hivResults$notifications,
      subsetRatesNational$annunique, 
      subsetRatesNational$deathrate,
      subsetRates$mrate,
      subsetRatesNational$propstay) %>%
      mutate(year = allYears) %>%
      select(year, everything()) %>%
      as_tibble()
    
    # Calculate National pldhiv by age
    pldhivOverallAge <- LivingDiagnosedAge(hivResultsNational,
      subsetRatesNational$annunique, 
      subsetRatesNational$deathrate,
      hivBase$migrationrate, 
      subsetRatesNational$propstay,
      agedeath = relAgeDeathNational,
      agemigrate = relAgeMigrateNational,
  ...
  if (doAge) {
    ...
    pldhivOveralllMin <- LivingDiagnosed(hivResults$notifications,
      subsetRates$annunique,  
      subsetRates$deathrate_upper,
      subsetRates$mrate_upper,
      subsetRates$propstay_lower) %>%
