...
if (interState) {
  ...
  if (doAge) {
    ...
    #  Use best estimate nationally (pldhivOverallAll) for min and max
...
# ECDC calculations-------------------------------------------------------
if (ecdcData) {
  normalizeFactor <- hivResults$include_notifications / 
    hivResults$notifcations
...
if (projectPldhiv) { 
  ...
  if (projectOption == "status-quo") {
    diagnosesFuture <- ProjVec(hivResults$notifcations, nprojYears)
    ...
    trendlm <- lm(diagnoses ~ year, 
      data.frame(year = (analysisYear - 4):analysisYear, 
        diagnoses = tail(hivResults$notifcations, 5)))
    trendDiags <- predict(trendlm, data.frame(year = projectYears))
    diagnosesFuture <- c(hivResults$notifcations, trendDiags)
  } else if(projectOption == "reduce") {
    diagnosesFuture <- projectDecreaseAll * 
      ProjVec(hivResults$notifcations, nprojYears)
  } else {
