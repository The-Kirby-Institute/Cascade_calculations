...
projectOutput <- "ASR_2025" # "ASR_2025" for latest ASR
# "HIV_Cascade_Paper"
...
if (is.null(paramsFile)) {
  ...
  nImputedSets <- rep(20, numCascades)  # Default is 20, the maximum
  ...
  ecdcExcludeRun <- rep(FALSE, numCascades) # Default FALSE to not run
  ...
  nImputedSets <- cascadeParameters$nImputedSets # Default: 20
  ...
  ecdcExcludeRun <- cascadeParameters$ecdcExcludeRun # Default: FALSE
...
if (calibration) {
  ...
  # Specify year of linkage for calibration
  linkageYear <- 2024
  
  # Change settings if this is a calibration run
  if (length(specificParams) > 1) {
    stop("Calibration should be for one set of parameters at a time.")
  }
      
  ecdcData[specificParams] <- FALSE 
  ecdcExcludeRun[specificParams] <- FALSE
  nImputedSets[specificParams] <- 1 # Try one first but need to do full set to validate calibration
...
for (ii in runCascades) {
  ...
  if (calibration) {
    print(paste(cascadeName[ii], "- Conducting calibration run"))
    
    # ecdcData[ii] <- FALSE 
    # ecdcExcludeRun[ii] <- FALSE
  ...
  if (!(targetLocalRegion[ii] == "all") & nrow(filter(absInterRegion, 
    ...
    if (ecdcExcludeRun[ii]){ 
      # Rerun but with the exclude options for the ECDC outputs set to true
      
      print(paste0(cascadeName[ii], " - ECDC Exclude"))
      
      pldhivCalculations_EcdcExclude <- list()
      
      pldhivCalculations_EcdcExclude[[ii]] <- CalculatePldhiv(
        analysisYear, saveResults, projectOutput, 
        # Cascade parameters
        cascadeName[ii], targetGender[ii], targetAge[ii], targetCob[ii], 
        targetExposure[ii], targetAtsi[ii], targetState[ii], targetLocalRegion[ii],
        ...
        # Specifications
        useImputed[ii], nImputedSets[ii], excludeAborig[ii], doRetained[ii], 
        doUnique[ii], yearUnique[ii], ecdcData[ii], ecdcVersion[ii], TRUE,
        TRUE, adjustmentStart[ii], adjustmentStop[ii], adjustStart[ii], 
        adjustStop[ii],
        # Projection specifications 
        projectPldhiv[ii], projectYear[ii], projectName[ii], projectOption[ii],
        projectDecrease[ii], 
...
```{r Generate calibration plot}
# Basic calibration plot to do a quick calibration needs to be verified with the
# full 2-HivCascadeLinkage.Rmd
...
if (calibration) {
  ...
  cascadePops <- read_csv(file.path(outputFolder, projectOutput, 
    paste0("cascadeNames-", analysisYear, ".csv")))
  
  cascadeCalib <- strsplit(cascadeName[specificParams],"-")[[1]][1] 
  
  # Set-up PBS data
  linkagePop <- cascadePops |> 
    filter(cascade == cascadeCalib) |>
    pull(linkage_pop)
  
  artpop <- tolower(cascadeCalib)
  
  if (artpop %in% c("all", "act", "nsw", "nt", "qld", "sa", "tas", "vic", "wa", "female", "male")) {
    pbsPopulation <- TRUE
  } else {
    stop("Can't calibrate without PBS data.")
  }
  ...
  linkageDataFile <- file.path(outputFolder,
    paste0("HIV_Linkage_Broad_high_1-", linkageYear, ".csv"))
  
  paramsFile <- file.path(outputFolder, projectOutput, 
    paste0(cascadeCalib, "-", analysisYear), "PldhivParameters.csv")
  
  artFile <- file.path(outputFolder,
    paste0("HIVtreatment-", analysisYear, "-PBS-Ineligible.csv"))
  ...
  pldhiv <- pldhivCalculations[[specificParams]]$pldhivAll  |> 
    filter(year %in% startYear:endYear) |>
    dplyr::select(year, pldhiv)
  
  # Calculate cascade steps and gaps
  diagnosed <- pldhiv |> pull(pldhiv)
  treated <- art$value
  
  cascadeGaps <- tibble(year = startYear:endYear, 
    art_estimate = treated / diagnosed) |>
    left_join(data |> select(-retained), by = "year") |> # merge in data
    pivot_longer(2:3) 
  
  # Produce calibration plot
  treatedPlot <- ggplot(data = cascadeGaps,
  ...
  # print out adjustments for record
  print(adjustmentStart[specificParams])
  print(adjustmentStop[specificParams])
