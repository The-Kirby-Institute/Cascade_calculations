...
This script is used for calculating the number of people living with
diagnosed HIV in Australia. This is one of the key steps for producing the
estimates of the Australian HIV diagnosis and care cascade. 

This script is currently in draft form for updating the HIV cascade 
...
cascadeName <- paste0("TAS-", toString(analysisYear)) 
targetGender <- 'all' # all, male, female
targetAge <- 'all' #groups 0, 1, 2, 3, 4...
targetCob <- 'all' # all=including n/a, Thailand, etc.
targetExposure <- 'all' #msm, hetero, pwid, otherexp
targetAtsi <- 'all' # indigenous or non_indigenous # Only used if country is Australia
targetState <- 'tas' # nsw, sa, nt, qld, vic, wa, act, tas 
targetLocalRegion <- 'all' #tbd
targetGlobalRegion <- 'all' #South-East Asia, Sub-Saharan Africa, Oceania, South American, Other cob, etc

# Script options for calculations
interState <- ifelse(targetState != "all", TRUE, FALSE)
excludeAborig <- FALSE
doAge <- FALSE # TRUE for aging calculations
...
source(file.path(HIVcode,"LivingDiagnosed.R"), echo=TRUE)

# Function to easily extract sub-populations of interest 
source(file.path(HIVcode, "SubHivSet.R"), echo=TRUE)
...
if (doAge) {
  ...
  #fill missing years
  fillYears <- includeDiags$year[!(includeDiags$year %in%
                                     agedDiags$yeardiagnosis)]
...
if (interState) {
  ...
  pldhivOverallMin <- LivingDiagnosed(adjustDiags$adjusted_included,
                                  subsetRatesAll$propunique,  
...
if (ecdcData) {
  ...
  # excluding OS diagnosis
    dataAll <- EcdcFolders(outputFolder, ecdcModel, 
                                  includeOS = FALSE)
    EcdcFiles(hivSet, dataAll, propUnique = subsetRates$propunique)
  } else {
