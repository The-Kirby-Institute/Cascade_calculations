...
*Current projects*
- ASR_2016, ..., ASR_2022 (cascades for the Annual Surveillance Report(ASR))
...
projectOutput <- "Testing" # "Undiagnosed_Metrics_Paper" # "ASR_2022" for latest ASR
                            # "HIV_Cascade_Paper"
...
# which need to exist
paramsFile <-  NULL # "ProjectParameters.csv"
...
if (is.null(paramsFile)) {
  ...
  cascadeName <- "male-2021" # (single)
  targetGender <- rep("male", numCascades) # all, male, female (single)
  ...
  # doAge below. 
  targetCob <- rep("non-australia", numCascades) # all=including n/a, Thailand, etc.
  ...
  excludeAborig <- rep(FALSE, numCascades) # FALSE by default to include 
                                           # indigenous
  doRetained <- rep(FALSE, numCascades) # TRUE for appending retained in care
  ...
  excludeOS <- rep(TRUE, numCascades) # Default FALSE to obtain total living 
                                      # with diagnosed HIV. Exclude previous 
                                      # diagnosed overseas and newly acquired 
