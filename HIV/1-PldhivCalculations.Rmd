...
cascadeName <- "Impute-2017" # (single)
...
targetState <- rep("vic", numCascades) # nsw, sa, nt, qld, vic, wa, act
  # tas (combination)
...
# 
useImputed <- TRUE
nImputedSets <- 10 # Default is 10, the maximum
...
if (useImputed) {
  ...
  for (ii in 1:nImputedSets) {
    ...
    if (interState && doAge) {
      tempHivSetReturnAll <- SubHivSet(hivSetAll, "all", targetGender,
      ...
      tempHivSetUnknownAll <- tempHivSetReturnAll[[3]]
    ...
    if (doUnique) {
      tempHivSetUnique <- SubHivSet(tempHivSet, "all", targetGender,
      ...
      if (interState) {
        tempHivSetUniqueAll <- SubHivSet(tempHivSet, "all", targetGender,
  ...
  ## number unknown and the number excluded when it uses a combination of 
