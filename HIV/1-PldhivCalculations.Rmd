...
if (interState) {
  ...
  if (doAge) {
    ...
    # Victoria testing
    viCSubset <- subsetRates
    # viCSubset$deathrate <- subsetRatesAll$deathrate
    # zeros <- rep(0,16)
    # viCSubset$inter_departrate <- 0.9 * viCSubset$inter_departrate
    # viCSubset$inter_arriverate <- 1.1 * viCSubset$inter_arriverate
    # viCSubset$mrate <- subsetRatesAll$mrate
    # viCSubset$mrate <- 0
    ...
    pldhivAll <- LivingDiagnosed(adjustDiags$adjusted_included,
      ...
      viCSubset$mrate,
      viCSubset$propstay, 
      arrivals = viCSubset$inter_arriverate,
      departs = viCSubset$inter_departrate,
      pldhiv = pldhivOverall$pldhiv) %>%
      mutate(year = allYears) %>%
      select(year, everything()) %>%
      as_tibble()

    pldhivAll %>% filter(year >= 2000)
