...
#                     toString(resultsYear), ".rda", sep = "")))
# hivSet <- read.csv(file.path(resultsFolder, paste("HIVset-",
#                     toString(resultsYear), ".csv", sep = "")))
...
if (pubPlots) {
  ...
  #                     toString(resultsYear), ".rda", sep = "")))
  # hivResults <- read.csv(file.path(resultsFolder, paste("HIVresults-",
  #                     toString(resultsYear), ".csv", sep = "")))
  
  # Load and extract specific results for all for publications plots
...
# Diagnostics-------------------------------------------------------------
# Aimed to produce a number of diagnostic of the poisson fit. Ended up 
...
# Trend plots ------------------------------------------------------------
# If doing pubplots create a second set of time trend plots with the 
...
if (pubPlots) {
  ...
  for (ii in plotStages) {
    ...
    local({
      ...
      plotsTrend[[ii]] <<- ggplot(data = tempData, aes(x = year, 
        ...
        ylab(yLabels[ii]) + xlab("Year") +  
        plotOpts +
        geom_text(x = 2012, y = 200, label = paste("Pseudo-R^2:", 
    ...
    ggsave(file.path(figFolder, paste0("HIVcascade-Trends-", ii, "_",
      toString(startYear), "-", toString(resultsYear),
      saveFormat)),
      plot = plotsTrend[[ii]], width = 7.5, height = 6, units = "cm")
  ...
  stagesDiffsPlot <- plot_grid(plotsTrend[[5]], plotsTrend[[6]],
                               plotsTrend[[7]],
          labels = c("A", "B", "C"), ncol = 2, nrow = 2, align = "v")
...
# Rename second column
finalTable[finalTable$population == "ausnonatsi",]$population <- "Australian born non-indigenous"
finalTable[finalTable$population == "atsi",]$population <- "Australian born indigenous"
finalTable[finalTable$population == "sea",]$population <- "Born in South East Asia"
finalTable[finalTable$population == "ssa",]$population <- "Born in sub-Saharan Africa"
