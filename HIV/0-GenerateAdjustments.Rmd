...
rawNotifications <- read.csv(file.path(notificationsFolder,
  paste0("cascadeHIVnotifications-clean-", toString(analysisYear), ".csv"))) 
...
if (analysisYear >= 2017) {
  ...
  # At this stage use last years migration rate for 2017
  mrate <- c(mrate, tail(mrate, 1))
...
# Specific NSW estimates - NSW data from 6-month follow-up
hivCobCum <- hivSet %>% 
  filter(state == "nsw") %>%
...
propLeaveNSWaus <- 0.0072 # Australian born
propLeaveNSWos <- 0.1008 # Overseas born
propStayNSW <- propAus * (1 - propLeaveNSWaus) + (1 - propAus) * 
  (1 - propLeaveNSWos)
propStayLowerNSW <-  propAus * (1 - 2 * propLeaveNSWaus) + (1 - propAus) *
  (1 - 2 * propLeaveNSWos)
...
# hivAdjustments$pstay_nswaus <- rep(1 - propLeaveNSWaus, length(allYears)) / hivBaseRates$propstay
hivAdjustments$pstay_nswaus <- ((1 - propLeaveNSWaus) / 
  tail(propStayNSW, 1)) * propStayNSW / hivBaseRates$propstay
hivAdjustments$pstay_nswaus_lower <- rep((1 - 2 * propLeaveNSWaus),
  length(allYears)) / hivBaseRates$propstay
hivAdjustments$pstay_nswaus_upper <- hivBaseRates$propstay_upper /
  hivBaseRates$propstay_upper

# hivAdjustments$pstay_nswos <- rep(1 - propLeaveNSWos, length(allYears)) /
#   hivBaseRates$propstay
hivAdjustments$pstay_nswos <- ((1 - propLeaveNSWos) / 
  tail(propStayNSW, 1)) * propStayNSW / hivBaseRates$propstay
hivAdjustments$pstay_nswos_lower <- rep((1 - 2 * propLeaveNSWos),
  length(allYears)) / hivBaseRates$propstay
hivAdjustments$pstay_nswos_upper <- hivBaseRates$propstay_upper /
  hivBaseRates$propstay_upper

# Australia COB adjustments - approximately half NSW
hivAdjustments$pstay_aus <- rep(1 - 0.5 * propLeaveNSWaus, 
  length(allYears)) / hivBaseRates$propstay
hivAdjustments$pstay_aus_lower <- hivAdjustments$pstay_nswaus
hivAdjustments$pstay_aus_upper <- hivBaseRates$propstay_upper /
  hivBaseRates$propstay_upper

hivAdjustments$pstay_os <- rep(1 - 0.5 * propLeaveNSWos, 
  length(allYears)) / hivBaseRates$propstay
hivAdjustments$pstay_os_lower <- hivAdjustments$pstay_nswos
hivAdjustments$pstay_os_upper <- hivBaseRates$propstay_upper /
  hivBaseRates$propstay_upper
