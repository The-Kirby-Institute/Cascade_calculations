...
ageDiagMale <- hivGenderAgeCum %>%
  select(year, agebin, male, propmale) %>%
  group_by(year) %>%
  arrange(year) %>%
  filter(agebin != "not_reported") %>%
  ...
  group_by(year, agebin) %>%
  mutate(diags = propdiags * notifications * propmale) %>%
  select(-propdiags, -notifications, -propmale) %>%
  spread(year, diags) %>%
  ungroup() %>%
  slice(c(1,10,2:9,11:18)) %>%
  select(-agebin) %>%
...
ageDiagFemale <- hivGenderAgeCum %>%
  select(year, agebin, other, propmale) %>%
  group_by(year) %>%
  arrange(year) %>%
  filter(agebin != "not_reported") %>%
  ...
  group_by(year, agebin) %>%
  mutate(diags = propdiags * notifications * (1 - propmale)) %>%
  select(-propdiags, -notifications, -propmale) %>%
  spread(year, diags) %>%
  ungroup() %>%
  slice(c(1,10,2:9,11:18)) %>%
