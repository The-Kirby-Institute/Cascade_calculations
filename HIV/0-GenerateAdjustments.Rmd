...
if (analysisYear >= 2022) {
  ...
  # 2017-Analysis year becaue the numbers closerly align. 
  ...
  # Generate final mrate - for 2022 the post-covid data is missing for the 
  # year 2022 assume same data as for 2019 (pre-covid)
  absDepartsMerged <- bind_rows(mrateAll, mrateAus |> filter(year >= 2017))
  absDepartsMerged <- bind_rows(
    absDepartsMerged, 
    tibble(
      year = 2022,
      departures = filter(absDepartsMerged, year == 2019)$departures,
      migrate = filter(absDepartsMerged, year == 2019)$migrate
    )
  )
