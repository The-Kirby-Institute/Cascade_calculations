...
hivData <- 
  TidyNotifications(
    rawNotifications, analysisYear, countryCodes, regionCodes
  ) 

if (sum(is.na(hivData$yeardiagnosis)) > 0) {
  # This should be zero. 2024 has one notification with missing year
  warning("Some notifications have missing year of diagnosis")
  print(sum(is.na(hivData$yeardiagnosis)))
  
  # Manually change the few missing or spit out error
  if (analysisYear == 2024) {
    # For 2024 one notification has missing yeardiagnosis 
    # Set year diagnosis to 2024 manually
    hivData$yeardiagnosis[is.na(hivData$yeardiagnosis)] <- 2024
  } else {
    stop("Notifications with missing year of diagnosis") 
  }
}
...
# Do cumulative country of birth by state as well for state cascades
hivCobCumState <- hivSet %>% 
  group_by(countrygroup, yeardiagnosis, state) %>% 
  summarise(notifications = n()) %>% 
  ungroup() %>% 
  group_by(countrygroup, state) %>% 
  mutate(cumnotifications = cumsum(notifications)) %>%
  ungroup() %>% 
  select(-notifications) %>%
  group_by(state) %>%
  spread(countrygroup, cumnotifications) %>%
  select(-`<NA>`) %>%
  gather("countrygroup", "cumnotifications", 3:6) %>%
  mutate(cumnotifications = ifelse(is.na(cumnotifications), 0, 
    cumnotifications)) %>%
  group_by(yeardiagnosis, state) %>% 
  mutate(propnotifications = cumnotifications / sum(cumnotifications)) %>%
  mutate(propnotifications = ifelse(is.na(propnotifications), 0, 
    propnotifications)) %>%
  select(-cumnotifications) %>%
...
if (analysisYear >= 2023) {
  # Issues with lastest data
  warning("Don't have updated data for 2023-2024. Will assume same values as 2022.") 
}

if (analysisYear >= 2022) {
  # Will use overall NOM departures and Australian citizen departures to 
  # estimate the migration rate
  absMigrationAus <- absMigration |> filter(grouping == "australian")
  absMigration <- absMigration |> filter(grouping == "total") 
  
}

if (analysisYear >= 2017) {
  # Given issues with >= 2017 data for permanent removals use 2016 data and
  # then add estimates up to the current year
  absDeparts2016 <- read.csv(file.path(mainDataFolder, 
...
if (analysisYear >= 2022) {
  # Use 2016 data and then appended Australian Citizen NOM departures data for
  # 2017-Analysis year because the numbers closely align. 
  
  absDeparts2016 <- mutate(absDeparts2016, migrate = permanent / erp)
  mrateAll <- absDeparts2016 |> 
    select(year, departures = permanent, migrate)
  
  absDepartsAus <- absMigrationAus |>
    filter(state == "all") |>
    mutate(migrate = departures / erp)
  mrateAus <- absDepartsAus |>
    select(year, departures, migrate)
  
  # Save a combined data set for testing and comparison
  if (plotMigrationData) {
    testMrate <- 
      bind_rows(
        mutate(mrateAll, type = "all"), 
        mutate(mrateAus, type = "aus")
      ) |>
      filter(year >= 2004)
    
    plotMrate <- ggplot(data = testMrate, 
      aes(x = year, y = departures, colour = type)) +
      geom_line() + 
      expand_limits(y = 0) +
      scale_colour_manual("",
        limits = c("all","aus"),
        values = c("blue", "red"),
        labels = c("Permanent departures", "Australian citizen NOM departures")) +
      xlab("Year") + ylab("Departures") +
      PlotOptions()
    
    plotMdata <- ggplot(data = absDeparts, aes(x = year)) +
      geom_line(aes(y=permanent, colour = "permanent")) + 
      geom_line(aes(y=resident, colour = "resident")) +
      geom_line(aes(y=visitor, colour = "visitor")) +
      geom_line(data = absDepartsAus, aes(y = departures, colour = "ausnom")) +
      geom_line(data = absDeparts2016, aes(y = permanent, colour = "2016")) +
      scale_colour_manual("Departures",
        limits = c("permanent","resident", "visitor", "2016", "ausnom"),
        values = c("black", "blue", "red", "purple", "darkgreen"),
        labels = c("Permanent", "Resident long-term", "Visitor long-term", 
          "Permanent up to 2016", "Australian citizen\nNOM departures")) +
      xlab("Year") + ylab("Number of departures") +
      PlotOptions() + theme(legend.position = "right")
    
    ggsave(file.path(dataFolder, 
      paste0("Migration_Permanent_NOM_Test-", analysisYear,".png")), 
      plot = plotMrate, width = 15, height = 10, units = "cm")
    
    ggsave(file.path(dataFolder, 
      paste0("Migration_Variables_Data-", analysisYear,".png")), 
      plot = plotMdata, width = 15, height = 10, units = "cm")
    
    write_csv(
      testMrate, 
      file.path(dataFolder, 
        paste0("Migration_Permanent_NOM_Test-", analysisYear,".csv")
      )
    )
  }
  
  # Generate final mrate - the post-covid data is missing for the 
  # year 2023-2024 assume same data as for 2022 (pre-covid)
  absDepartsMerged <- bind_rows(mrateAll, mrateAus |> filter(year >= 2017))
  absDepartsMerged <- bind_rows(
    absDepartsMerged, 
    tibble(
      year = 2023:2024,
      departures = filter(absDepartsMerged, year == 2022)$departures,
      migrate = filter(absDepartsMerged, year == 2022)$migrate
    )
  )
  
  mrate <- absDepartsMerged$migrate
  
} else if (analysisYear %in%  2017:2021) {
  # Use 2016 data and then estimate rate for following years. Estimates
  # are generated using the NOM Departures data with an adjustment factor 
  # estimated using the data for 2004-2016. These estimates and associated
  # calculations are provided in the file:
  #  ~\HIV\data\Migration_Permanent_NOM_Comparison.xlx
  #  This file needs updating each year
  
  absDeparts <- mutate(absDeparts, migrate = permanent / erp)
  mrate <- absDeparts$migrate
  
  # Read in Excel file and extract estimates for permanent departure rate 
  # after 2017. NEEDS TO BE UPDATED ANNUALLY
  estimatedMrate <- read_xlsx(file.path(dataFolder,
    "Migration_Permanent_NOM_Comparison_2017-2021.xlsx"), skip = 3) %>%
    select(year = Year, mrate = "Base (Permanent)") 
  
  if (tail(estimatedMrate$year, 1) < analysisYear) {
    stop("Need to update estimated migration rate")
  }
  
  estimatedMrate <- estimatedMrate %>%
    filter(year >= 2017, year <= analysisYear)
  
  mrate <- c(mrate, estimatedMrate$mrate)
...
# contain 2015 data but this is only for first six months. 
...
# from the ABS. WARNING ERP unavailable in 5 year bins for > 75 years old 
...
# jurisdictions or longer periods. We assume this movement has occurred in 
# other jurisdictions and been mostly constant for the whole epidemic though 
# less in the early stages of the epidemic as there were less overseas born 
# diagnosed. There is lots of uncertainty and limited data but we use the
# data for Australian born and overseas born from NSW to estimate this
# post-diagnosis movement for east state and territory and Australia overall. 

# Specific NSW estimates (hardcoded) -- based on 6-monthly follow-up data 
# from NSW Health cumulatively over 2013-2019, 2020-2021 (COVID era), and 2022 
# (post COVID). We separate the pre-COVID and COVID data given the massive 
# interruption to migration that occurred during the COVID period. 
# 
# Data for 2013-2022 in the local file 
# ~\Australian Care Cascades - Documents\data\6m_followUp_NSW_Oct2023.xlsx.
# The variables below store the cumulative emigration proportion for 
# Australian born diagnosed and overseas born to end 2022 with the COVID data 
# separated out.  
ausBornEmigrate19 <- 0.0076923
osBornEmigrate19 <- 0.0904007

ausBornEmigrateCOVID <- 0.0
osBornEmigrateCOVID <- 0.0298507

ausBornEmigrate2022 <- 0.01369863
osBornEmigrate2022 <- 0.023809524

ausBornEmigrate <- 
  c(
    rep(ausBornEmigrate19, length(startYear:2019)),
    rep(ausBornEmigrateCOVID, length(2020:2021)), 
    rep(ausBornEmigrate2022, length(2022:analysisYear))
  ) 

osBornEmigrate <- 
  c(
    rep(osBornEmigrate19, length(startYear:2019)),
    rep(osBornEmigrateCOVID, length(2020:2021)),
    rep(osBornEmigrate2022, length(2022:analysisYear))
  ) 

# Overall estimates
propAus <- hivCobCum$australia

hivBaseRates$propstay <- propAus * (1 - ausBornEmigrate) + (1 - propAus) * 
  (1 - osBornEmigrate)
hivBaseRates$propstay_lower <- propAus * (1 - 2 * ausBornEmigrate) + (1 - propAus) * 
  (1 - 2 * osBornEmigrate)
hivBaseRates$propstay_upper <- rep(1, length(allYears))

# Adjust overall reates for Indigenous vs non-Indigenous
...
hivAdjustments$pstay_aus <- (1 - ausBornEmigrate) / hivBaseRates$propstay
hivAdjustments$pstay_aus_lower <- (1 - 2 * ausBornEmigrate) / 
  hivBaseRates$propstay_lower
hivAdjustments$pstay_aus_upper <- rep(1, length(allYears)) / 
  hivBaseRates$propstay_upper

hivAdjustments$pstay_os <- (1 - osBornEmigrate) / hivBaseRates$propstay
hivAdjustments$pstay_os_lower <- (1 - 2 * osBornEmigrate) / 
  hivBaseRates$propstay_lower
hivAdjustments$pstay_os_upper <- rep(1, length(allYears)) / 
  hivBaseRates$propstay_upper

# NSW estimates
propAusNSW <- filter(hivCobCumState, state == "nsw")$australia 

propStayNSW <- propAusNSW * (1 - ausBornEmigrate) + (1 - propAusNSW) * 
  (1 - osBornEmigrate)
propStayLowerNSW <-  propAusNSW * (1 - 2 * ausBornEmigrate) + (1 - propAusNSW) * 
  (1 - 2 * osBornEmigrate)
...
# Victorian estimates - Victoria had no notifications in 1981 and 1982
propAusVic <- FillDataFrame(allYears, filter(hivCobCumState, state == "vic"), 
  cumulative = TRUE)$australia 

propStayVic <- propAusVic * (1 - ausBornEmigrate) + (1 - propAusVic) * 
  (1 - osBornEmigrate)
propStayLowerVic <-  propAusVic * (1 - 2 * ausBornEmigrate) + (1 - propAusVic) * 
  (1 - 2 * osBornEmigrate)
...
propAusQld <- FillDataFrame(allYears, filter(hivCobCumState, state == "qld"), 
  cumulative = TRUE)$australia 

propStayQld <- propAusQld * (1 - ausBornEmigrate) + (1 - propAusQld) * 
  (1 - osBornEmigrate)
propStayLowerQld <-  propAusQld * (1 - 2 * ausBornEmigrate) + (1 - propAusQld) * 
  (1 - 2 * osBornEmigrate)
propStayUpperQld <- rep(1, length(allYears))

hivAdjustments$pstay_qld <- propStayQld / hivBaseRates$propstay
hivAdjustments$pstay_qld_lower <- propStayLowerQld /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_qld_upper <- propStayUpperQld /
  hivBaseRates$propstay_upper

# Western Australia
propAusWa <- FillDataFrame(allYears, filter(hivCobCumState, state == "wa"), 
  cumulative = TRUE)$australia 

propStayWa <- propAusWa * (1 - ausBornEmigrate) + (1 - propAusWa) * 
  (1 - osBornEmigrate)
propStayLowerWa <-  propAusWa * (1 - 2 * ausBornEmigrate) + (1 - propAusWa) * 
  (1 - 2 * osBornEmigrate)
propStayUpperWa <- rep(1, length(allYears))

hivAdjustments$pstay_wa <- propStayWa / hivBaseRates$propstay
hivAdjustments$pstay_wa_lower <- propStayLowerWa /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_wa_upper <- propStayUpperWa /
  hivBaseRates$propstay_upper

# South Australia
propAusSa <- FillDataFrame(allYears, filter(hivCobCumState, state == "sa"), 
  cumulative = TRUE)$australia 

propStaySa <- propAusSa * (1 - ausBornEmigrate) + (1 - propAusSa) * 
  (1 - osBornEmigrate)
propStayLowerSa <-  propAusSa * (1 - 2 * ausBornEmigrate) + (1 - propAusSa) * 
  (1 - 2 * osBornEmigrate)
propStayUpperSa <- rep(1, length(allYears))

hivAdjustments$pstay_sa <- propStaySa / hivBaseRates$propstay
hivAdjustments$pstay_sa_lower <- propStayLowerSa /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_sa_upper <- propStayUpperSa /
  hivBaseRates$propstay_upper

# ACT
propAusAct <- FillDataFrame(allYears, filter(hivCobCumState, state == "act"), 
  cumulative = TRUE)$australia 

propStayAct <- propAusAct * (1 - ausBornEmigrate) + (1 - propAusAct) * 
  (1 - osBornEmigrate)
propStayLowerAct <-  propAusAct * (1 - 2 * ausBornEmigrate) + (1 - propAusAct) * 
  (1 - 2 * osBornEmigrate)
propStayUpperAct <- rep(1, length(allYears))

hivAdjustments$pstay_act <- propStayAct / hivBaseRates$propstay
hivAdjustments$pstay_act_lower <- propStayLowerAct /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_act_upper <- propStayUpperAct /
  hivBaseRates$propstay_upper

# Tasmania
propAusTas <- FillDataFrame(allYears, filter(hivCobCumState, state == "tas"), 
  cumulative = TRUE)$australia 

propStayTas <- propAusTas * (1 - ausBornEmigrate) + (1 - propAusTas) * 
  (1 - osBornEmigrate)
propStayLowerTas <-  propAusTas * (1 - 2 * ausBornEmigrate) + (1 - propAusTas) * 
  (1 - 2 * osBornEmigrate)
propStayUpperTas <- rep(1, length(allYears))

hivAdjustments$pstay_tas <- propStayTas / hivBaseRates$propstay
hivAdjustments$pstay_tas_lower <- propStayLowerTas /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_tas_upper <- propStayUpperTas /
  hivBaseRates$propstay_upper

# Northern territory
propAusNt <- FillDataFrame(allYears, filter(hivCobCumState, state == "nt"), 
  cumulative = TRUE)$australia 

propStayNt <- propAusNt * (1 - ausBornEmigrate) + (1 - propAusNt) * 
  (1 - osBornEmigrate)
propStayLowerNt <-  propAusNt * (1 - 2 * ausBornEmigrate) + (1 - propAusNt) * 
  (1 - 2 * osBornEmigrate)
propStayUpperNt <- rep(1, length(allYears))

hivAdjustments$pstay_nt <- propStayNt / hivBaseRates$propstay
hivAdjustments$pstay_nt_lower <- propStayLowerNt /
  hivBaseRates$propstay_lower
hivAdjustments$pstay_nt_upper <- propStayUpperNt /
  hivBaseRates$propstay_upper
...
# Load AHOD data to calculate mortality and do some simple 
ahodData <- read.csv(file.path(dataFolder, 
  paste0("ahod", analysisYear, ".csv")))

# TODO: Load gender deathrate ratios - NEED TO UPDATE THE ESTIMATES IN THIS FILE EACH
# YEAR
warning("Make sure you update the sexDrates regression to the analysisYear in the Excel Spreadsheet")
...
for (ii in 2:length(annDiags)) {
  ...
  # I had previously been using the old formula (with "new" formula commented 
  # out!). I don't think it makes a massive difference to the estimates but I 
  # think the new formula better captures what we want conceptually. 
  
  # TODO: It would be good to replace the calculation below with 
  # a function that is also used in LivingDiagnosed.R so that
  # accidental differences are prevented.
  
  # Old formulas: Based on an Euler approximation approach but given the
...
# Old formula Calculation of crude average annual mortality rates up to 2003
...
# New formula calculations of crude average annual mortality rates up to 2003
# This is given by number deaths in given year divided by previous year living
# with HIV estimate to match the new formula 
...
hivAdjustments$drate_qld_lower <- hivAdjustments$drate_qld_lower /
  deathrateDiv_lower
...
hivAdjustments$drate_qld_upper <- hivAdjustments$drate_qld_upper /
  deathrateDiv_upper

```

```{r Interstate migration rates}
# This chunk calculates interstate migration rates. At this stage just 
