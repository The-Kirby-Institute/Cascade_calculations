...
if (analysisYear >= 2022) {
  ...
  absDepartsMerged <- bind_rows(
    ...
    tibble(
      ...
      migrate = filter(absDepartsMerged, year == 2019)$departures
    )
  )
...
# Specific NSW estimates (hardcodede) -- based on 6-monthly follow-up data 
# from NSW Health cumulatively over 2013-2019 and 2020-2021 (COVID era). We 
# Separate the pre-COVID and COVID data given the massive interuption to 
# migration that occured during the COVID period. 
# 
# Data for 2013-2021 in the local file 
# ~\Australian Care Cascades - Documents\data\6m_followUp_NSW_Sep2022.xlsx.
# The variables below store the cumulative emigration proportion for 
# Australian born diagnosed and overseas born to end 2021 with the COVID data 
# separated out.  
...
ausBornEmigrate <- c(rep(ausBornEmigrate19, length(startYear:2019)),
  rep(ausBornEmigrateCOVID, length(2020:analysisYear))) 
osBornEmigrate <- c(rep(osBornEmigrate19, length(startYear:2019)),
  rep(osBornEmigrateCOVID, length(2020:analysisYear))) 
