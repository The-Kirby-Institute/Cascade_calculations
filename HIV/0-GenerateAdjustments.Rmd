...
for (ii in 2:nyears) {
  ...
  unknownTest <- sum(agePldhiv[, ii-1]) * hivBaseRates$deathrate[ii-1] -
    sum(ageDeath[, ii-1])
  # unknownTest <- numLiving[ii-1] * hivBaseRates$deathrate[ii-1] -
  #   sum(ageDeath[, ii-1])
  ...
  for (jj in 1:nages) {
    ...
    agePldhiv[jj, ii] <- agePldhiv[jj, ii - 1] + 
      hivBaseRates$annunique_all[ii] * hivResultsAge[jj, ii] - 
      (1-hivBaseRates$propstay[ii-1]) * hivBaseRates$annunique_all[ii-1] *
      hivResultsAge[jj, ii-1] - 
      # (1-hivBaseRates$propstay[ii-1]) * ageDiag[jj, ii-1] - 
