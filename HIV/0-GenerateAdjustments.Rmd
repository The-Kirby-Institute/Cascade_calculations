...
if (sum(is.na(hivData$yeardiagnosis)) > 0) {
  ...
  print(sum(is.na(hivData$yeardiagnosis)) > 0)
  ...
  hivData$yeardiagnsis[is.na(hivData$yeardiagnosis)] <- 2024
  } else {
...
if (analysisYear >= 2022) {
  ...
  if (plotMigrationData) {
    ...
    plotMrate <- ggplot(data = testMrate, aes(x = year, y = departures, colour = type)) +
      geom_line() + 
      expand_limits(y = 0) +
      scale_colour_manual("",
        limits = c("all","aus"),
  ...
  # year 2023 assume same data as for 2022 (pre-covid)
...
# TODO: TEMPORARY AHOD DATA USED FOR 2022 AS DATA NOT YET AVAILABLE - 27 June 2022
ahodData <- read.csv(file.path(dataFolder, 
  paste0("ahod", analysisYear, ".csv")))

# Load gender deathrate ratios - NEED TO UPDATE THE ESTIMATES IN THIS FILE EACH
# YEAR
