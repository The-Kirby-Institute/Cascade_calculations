...
pldhivFuture <- linearResults %>%
  rename(value = pldhiv) 
AverageAge(pldhivFuture, 2006)
AverageAge(pldhivFuture, 2016)
AverageAge(pldhivFuture, 2026)
...
paperFigurePlot <- grid.arrange(paperFigureA, paperFigureB, legend, 
  ncol = 3, nrow = 1,
  layout_matrix = rbind(c(1, 2, 3)),
  widths = c(4, 4, 0.8), heights = c(3))

paperFigure <- ggdraw() +
  draw_plot(paperFigurePlot, 0, 0, 1, 1) +
...
if (savePlots) {
  SaveFigure(figFolder, "Combined_paper_figure", paperFigure,
    format = saveFormat, width = 20, height = 10, units = units)
}
```
