...
cascadeProject <- "HIV_Cascade_Paper"
cascadeName <- "All"

# ASR_2025 c("All", "NonIndigenous", "AusNonIndigenous", "Indigenous", "Male", 
  # "Female", "SCA", "SSA", "SEA", "MSM", "MSM_AUS", "MSM_OS", "Hetero", "Pwid") 
  # # Don't have linkage data for Hetero_Aus and Hetero_OS yet. 
...
if (pbsPopulation) {
  ...
  cascadeRetained <- cascadeGaps |>
    filter(str_detect(name, "retained"), year <= endYear)
  cascadeTreat <- cascadeGaps |>
    filter(name %in% c("treated", "art_estimate"), year <= endYear)
  
  retainedPlot <- ggplot(data = cascadeRetained,
    aes(x = year, y = value, colour = name)) +
    geom_line() +
    scale_y_continuous(labels = percent) +
    scale_x_continuous(breaks = 2012:endYear, 
      labels = EveryNth(2012:endYear, 3, inverse = TRUE)) +
    ...
    geom_pointrange(aes(x = 2019, y = 0.9624, ymin = 0.93, ymax = 0.99), 
      colour = "black") + 
    expand_limits(y = c(0.8, 1), x = c(2012, endYear)) +
    xlab("Year") + ylab("Percent retained") +  
    PlotOptions()
  ...
  treatedPlot <- ggplot(data = cascadeTreat,
    ...
    geom_line() +
    scale_y_continuous(labels = percent) +
    scale_x_continuous(breaks = 2015:endYear, 
      labels = EveryNth(2015:endYear, 3, inverse = TRUE)) +
    scale_colour_manual(name = "",
      values = c("blue", "red"),
    ...
    expand_limits(y = c(0.8, 1), x = (2015:endYear)) +
    xlab("Year") + ylab("Percent on ART") +
    PlotOptions() 
...
if (savePlots) {
  ...
  if (pbsPopulation) {
    SaveFigure(figureFolder, 
      ...
      retainedPlot, width = 14, height = 12)
    
    SaveFigure(figureFolder, 
      paste0("Linkage_ART_Comparison-", linkageYear, "-Data"),
      treatedPlot, width = 14, height = 12)
...
if (extraPlots) {
  ...
  if (savePlots) {
    ...
    write_excel_csv(artProp, file.path(figureFolder, 
      paste0("ART_Coverage_Estimates-" , cascadeYear, 
      "-Data.csv")))
