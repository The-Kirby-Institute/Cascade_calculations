...
This document shows various plots related to the HIV diagnosis and care 
...
Note: Kniting this script only works for the ASR cascade projects. The 
script can be used to produce plots for specific cascades using the right 
script parameters.
...
# Script parameters ------------------------------------------------------
resultsYear <- 2022  # year of cascade we will plot
minYear <- 19 # how long ago do we plot things (from 2004)
startYear <- resultsYear - minYear + 1 

# Set up project - If ASR plot all populations otherwise just plot 
# specified cascade
projectName <- "ASR_2023" #"ASR_2023" "HIV_Cascade_Paper"
...
newInfectsPlots <- FALSE
linkageplots <- ifelse(pubPlots, TRUE, FALSE)
saveFormat <- ".png" # file extension for saved plots 
excludeCare <- FALSE # Include linked and retained in care?

# New infections parameters
infectsCalculation <- "exclude" # "include", "exclude" or "middle"

# Setup based on specs
asrcols <- PlotColors("hivcascade5")
if (projectName %in% c("ASR_2023", "ASR_2022", "ASR_2021", "ASR_2020", "ASR_2019", "ASR_2018", 
  "ASR_2017", "ASR_2016")) {
  # ASR do many populations otherwise do the specified one
  cascadePop <- "all" 
  asrCascades <- TRUE
} else if (projectName == "HIV_Cascade_Paper") {
  asrCascades <- FALSE 
  newInfectsPlots <- TRUE
} else {
  # Turn off newInfectsPlots
  asrCascades <- FALSE 
  newInfectsPlots <- TRUE
}
...
# Number of x-axis ticks in plots
xTicks <- 4
...
inputFile <- file.path(resultsFolder, 
...
if (newInfectsPlots) {
  ...
  newInfections <- read_csv(file.path(resultsFolder, 
    paste0("All-", resultsYear), 
    paste0("newInfectionsEstimates-", resultsYear,".csv")))
}

if (pubPlots) {
  # Load cleaned notifications data and tidy up for calculations
  ...
  # Load the previously saved results for the national cascade
  cascadeFolder <- file.path(resultsFolder, paste0("All-",
    toString(resultsYear)))
  
  pldhivAll <- read.csv(file.path(cascadeFolder, paste0("pldhiv-",
    toString(resultsYear), "-all.csv")))
  pldhivMin <- read.csv(file.path(cascadeFolder, paste0("pldhiv-",
    toString(resultsYear), "-min.csv")))
  pldhivMax <- read.csv(file.path(cascadeFolder, paste0("pldhiv-",
    toString(resultsYear), "-max.csv")))
  
  hivResults <- tibble(yeardiagnosis = pldhivAll$year)
  hivResults$deaths <- cumsum(pldhivAll$deaths)
  hivResults$deaths_min <- cumsum(pldhivMin$deaths)
  hivResults$deaths_max <- cumsum(pldhivMax$deaths)
  hivResults$emmigrants <- cumsum(pldhivAll$emigrants + pldhivAll$diag_departs)
  hivResults$emmigrants_min <- cumsum(pldhivMin$emigrants + pldhivMin$diag_departs)
  hivResults$emmigrants_max <- cumsum(pldhivMax$emigrants + pldhivMax$diag_departs)
  ...
  hivAdjustments <- read.csv(file.path(dataFolder,
    paste0("HIVadjustments-", toString(resultsYear), ".csv")))
  hivInterstate <- read.csv(file.path(dataFolder,
    paste0("HIVinterstateEstimates-", toString(resultsYear), ".csv")))
  
  # Now get the death rates, migration rates, and proportion stay rates
  subsetRates <- GetAdjustments(hivBase, hivAdjustments, "all", "all", 
    "all", "all", "all", "all", "all", "all")
  
  hivResults$propunique <- subsetRates$cumunique
  hivResults$deathrate <- subsetRates$deathrate
  hivResults$drlower <- subsetRates$deathrate_lower
  hivResults$drupper <- subsetRates$deathrate_upper
  hivResults$mrate <- subsetRates$mrate
  hivResults$mrate_lower <- subsetRates$mrate_lower
  hivResults$mrate_upper <- subsetRates$mrate_upper
  
  annualDiags <- hivSet %>% 
    group_by(yearhiv) %>%
    summarise(notifications = n(),
      numlinked = sum(linked == "yes")) %>%
    mutate(totalnotifications = cumsum(notifications),
      proplinked = numlinked / notifications)
  
  hivResults$notifications <- annualDiags$notifications
  hivResults$totalnotifications <- annualDiags$totalnotifications
  hivResults$numlinked <- annualDiags$numlinked
  hivResults$proplinked <- annualDiags$proplinked
  
  hivResults <- hivResults %>%
    mutate(uniquecases = totalnotifications * propunique)
}
...
resultsAllYear |>
  mutate(
    value = FormatData(value, places = -1), 
    lower = FormatData(lower, places = -1),
    upper = FormatData(upper, places = -1)
  ) |> 
  write_csv(
    file = file.path(figFolder, paste0("HIVcascade-all-", 
    resultsYear, ".csv"))
  )
...
# Caption
figs("plotBarAll", paste0("The HIV care and treatment cascade for 
  Australia in ", toString(resultsYear), "."))
...
if (pubPlots) {
  ...
  pldhiv <- hivCascade %>%
    filter(stage == "pldhiv", population == cascadePop) %>%
    select(-stage, -population) 
  
  plhiv <- hivCascade %>%
    filter(stage == "infected", population == cascadePop) %>%
    select(-stage, -population)
  ...
  plotDeaths <- ggplot(data = hivResults, aes(x = yeardiagnosis, 
    y = deaths)) +
    geom_ribbon(aes(ymin = deaths_min, ymax = deaths_max), 
      fill = asrcols[2], alpha = 0.4) + 
    geom_line(color = asrcols[2])  +
    scale_x_continuous(breaks = seq(startYear, resultsYear),
      labels =  rev(EveryNth(seq(resultsYear, startYear), xTicks, inverse = TRUE)),
  ...
  plotDeathRate <- ggplot(data = hivResults, 
  ...
  # Plot emigration related data - used same approach as for deaths reporting 
  # emigrants among diagnosed
  
  # Includes post-diagnosis emigrants
  plotMigrants <- ggplot(data = hivResults, aes(x = yeardiagnosis, 
    y = emmigrants)) +
    geom_line(color = asrcols[2])  +
    scale_x_continuous(breaks = seq(startYear, resultsYear),
      labels =  rev(EveryNth(seq(resultsYear, startYear), xTicks, inverse = TRUE)),
    ...
    expand_limits(y = 0) +
    scale_y_continuous(labels = comma) + 
    ylab("Number of emmigrants") + xlab("Year") +  
    PlotOptions()
  
  # Only permanent removals
  plotMigrationRate <- ggplot(data = hivResults, 
    aes(x = yeardiagnosis, y = mrate)) +
    geom_line(color = asrcols[2])  +
    scale_x_continuous(breaks = seq(startYear, resultsYear),
      labels =  rev(EveryNth(seq(resultsYear, startYear), xTicks, inverse = TRUE)),
      limits = c(startYear, resultsYear)) +
    scale_y_continuous(labels = percent, limits = c(0, 0.01)) + 
    ylab("Emmigration rate (%)") + xlab("Year") +  
    PlotOptions()
  ...
  ggsave(file.path(figFolder, paste("HIVmigrants_", 
    toString(startYear), "-", toString(resultsYear), 
    saveFormat, sep = "")),
    plot = plotMigrants, width = 7.5, height = 6, units = "cm") 
  
  ggsave(file.path(figFolder, paste("HIVmigration_rate_", 
    toString(startYear), "-", toString(resultsYear), 
    saveFormat, sep = "")),
    plot = plotMigrationRate, width = 7.5, height = 6, units = "cm")
  
  # Create a combined plot
  HIVdiagnosedPlot <- ggdraw() +
    draw_plot(plots[[2]], 0, 0.66, 0.5, 0.33) + #Number PLDHIV
    draw_plot(casesPlot, 0.5, 0.66, 0.5, 0.33) +
    draw_plot(plotDeaths, 0, 0.33, 0.5, 0.33) +
    draw_plot(plotDeathRate, 0.5, 0.33, 0.5, 0.33) +
    draw_plot(plotMigrants, 0, 0, 0.5, 0.33) +
    draw_plot(plotMigrationRate, 0.5, 0, 0.5, 0.33) +
    draw_plot_label(c("A", "B", "C", "D", "E", "F"), c(0, 0.5, 0, 0.5, 0, 0.5), 
      c(1, 1, 0.66, 0.66, 0.33, 0.33), size = 12)
  
  ggsave(file.path(figFolder, paste("HIVdiagnosed_indicators_", 
    toString(startYear), "-", toString(resultsYear), 
    saveFormat, sep = "")),
    plot = HIVdiagnosedPlot, width = 20, height = 24, units = "cm")
  
  # Treatment coverage plot ----------------------------------------------
  
  # Do our calculations and sort out data we want to plot
  treatData <- hivCascade %>%
    filter(stage == "numART", population == cascadePop) %>%
    ...
    mutate(artcov = ifelse(artcov > 1, 1, artcov), 
      artcov_min = ifelse(artcov_min > 1, 1, artcov_min), 
      artcov_max = ifelse(artcov_max > 1, 1, artcov_max)) %>%
  ...
  suppressData <- hivCascade %>%
    filter(stage == "suppressed", population == cascadePop) %>%
...
if (newInfectsPlots) {
  ...
  infectsYear <- startYear # resultsYear - 10 + 1 # Last 10 years
  
  # Sort out new infections
  if (infectsCalculation == "include") {
    infectsData <- filter(newInfections, year >= infectsYear,
      exclude == "no") %>%
      select(-exclude, estimate = infections, lower = infections_lower, 
    ...
    infectsData <- tibble(year = infectsYear:resultsYear)
    
    infectsData$estimate <- (filter(newInfections, year >= infectsYear,
      exclude == "no")$infections + filter(newInfections,
        year >= infectsYear, exclude == "yes")$infections) / 2
    
    infectsData$lower <- filter(newInfections, year >= infectsYear,
      exclude == "yes")$infections_lower
    
    infectsData$upper<- filter(newInfections, year >= infectsYear,
      exclude == "no")$infections_upper
  }
  ...
  # Add number of notifications/diagnoses unadjusted for duplicates excluding 
  # previously diagnosed overseas for comparison
  
  diagsData <- hivSet %>% 
    group_by(yearhiv) %>%
    filter(is.na(previ_diag_overseas)) %>%
    summarise(diags = n()) %>%
    filter(yearhiv >= infectsYear) %>%
    ungroup() %>%
    rename(year = yearhiv)
  ...
  plotRelative <- ggplot(data = infectsData, aes(x = year)) +
    ...
    ylab(paste0("Relative to ", infectsYear)) + xlab("Year") +
    PlotOptions() + 
  ...
  ggsave(file.path(figFolder, paste("HIV_new_infections_",
    toString(infectsYear), "-", toString(resultsYear),
    saveFormat, sep = "")),
  ...
  trendsInfections <- tibble(stage = c("infections", "rate"),
  ...
  write_csv(trendsInfections, file.path(figFolder,
    paste0("Trends_infections_",
      toString(infectsYear), "-", toString(resultsYear), ".csv")))
}
```

```{r linkageplots}
# Create  a series of plot related to linkage to care for publication

if (asrCascades && linkageplots) {
  
  # Proportion linked -----------------------------------------------------
  
  # Do some organization and calculations so we can compare MSM to non-MSM 
  uniqueDiags <- c(hivResults$uniquecases[1],
    diff(hivResults$uniquecases))
  uniqueDiags <- tail(uniqueDiags, minYear)
  ...
  propUnique <- tail(propUnique, minYear) # 14 because of use of diff()
  # 2004-2018
  
  # Create data frame for unique cases for each group excluding unknowns
  expDiags <- hivSet %>% 
    group_by(expgroup, yeardiagnosis) %>% 
    summarise(notifications = n()) %>%
    mutate(totalnotifications = cumsum(notifications)) %>%
    filter(yeardiagnosis <= resultsYear,
      yeardiagnosis >= startYear, !is.na(expgroup)) %>%
  ...
  # each exposure group - issues with using unique cases due to 
  # differences in the cumulative number uniquw being negative !!
  # Therefore use notifications overall for calculations. 
  ...
  expLinked <- hivSet %>% 
    ...
    mutate(proplinked = linked/notifications)
  
  nonMsmLinked <- expLinked %>%
    filter(expgroup != "msm") %>%
    group_by(year) %>%
    summarise(linked = sum(linked), uniquecases= sum(uniquecases),
  ...
  hivResults %>% 
    filter(yeardiagnosis >= 1984) %>% 
    select(yeardiagnosis, proplinked) %>%
    write_csv(file.path(figFolder, paste0("linkage_prop_", 
  ...
  linkPlotRecent <- ggplot(data = hivResults,
    aes(x = yeardiagnosis, y = proplinked)) + 
    geom_line(aes(colour = "all"))  +
    geom_line(data = filter(expLinked, expgroup == "msm"),
      aes(x = year, y = proplinked, colour = "msm")) +
    geom_line(data = nonMsmLinked, aes(x = year, y = proplinked,
      colour = "nonmsm")) +
  ...
  ggsave(file.path(figFolder, paste("linkage_prop_", 
    ...
    plot = linkPlot, width = 9, height = 7.5, units = "cm")
  ...
  linkDays <- hivSet %>%
    ...
    filter(dayslinked >= 0) %>%
    mutate(weeklinked = ceiling(dayslinked/7)) %>%
    mutate(weeklinked = ifelse(weeklinked == 0, 1, weeklinked))
  ...
  # Plot distributions
  linkDist <- ggplot(data = linkDays, aes(x = weeklinked)) +     # bin days into weeks
    geom_freqpoly(binwidth = 1, aes(colour = "all")) + 
    geom_freqpoly(data = msmLinkDays, 
      aes(x = weeklinked, colour = "msm"), 
      binwidth = 1) +
    geom_freqpoly(data = nonMsmLinkdays, 
      aes(x = weeklinked, colour = "nonmsm"), 
      binwidth = 1) +
  ...
  # Need to remove those previously diagnosed overseas first as they maybe
  ...
  # Gather into long format for plotting and save separately
  allCd4 <- gather(allCd4, "category", "cd4", 2:4)
  
  write_csv(allCd4, file.path(figFolder, paste0("linkage_CD4_", 
    toString(startYear), "-", toString(resultsYear), ".csv")))
  ...
  # Save plot
  ggsave(file.path(figFolder, paste0("linkage_CD4_", 
    toString(startYear), "-", toString(resultsYear), 
    saveFormat)), plot = aveCd4Plot, 
    width = 9, height = 7.5, units = "cm")
  
  # CD4 count distribution comparison ------------------------------------------
  
  # Compare distribution of CD4 count when diagnoses highest and results year
  # 1,079 notifications (excluding previous diagnosis in 2014). 
  # CD4 counts > 2000 have been filtered out. 
  
  cd4Data <- cd4Set %>% 
    filter(yearhiv %in% c(2014,2021), !is.na(cd4count), cd4count <=2000) %>%
    select(year = yearhiv, cd4count) %>%
    mutate(year = as.factor(year))
  
  cd4Hist <- ggplot(data = cd4Data, aes(x = cd4count, colour = year, group = year)) + 
    geom_freqpoly(binwidth = 50) +
    scale_colour_manual(name = "", 
      breaks = c("2014", "2021"),
      values = c(asrcols[2], asrcols[4])) +
    ylab("Count") + xlab("CD4 count at diagnosis") +
    PlotOptions()
  
  # Save plot
  ggsave(file.path(figFolder, paste0("CD4_histogram", 
    toString(startYear), "-", toString(resultsYear), 
    saveFormat)), plot = cd4Hist, 
    width = 9, height = 7.5, units = "cm")
...
# Caption
tabs("finalTable", paste0("Estimates and range for each stage of the HIV care and treatment cascade for Australia in ", toString(resultsYear), "."))

```

`r tabs("finalTable")`
```{r inserttable1, include = TRUE, results = "asis"}
kable(finalTable, align = c("l", "c", "c"), row.names = FALSE)
```
...
# Caption
figs("plotbarAllStates", paste0("The HIV care and treatment cascade for each State and Territory in ", toString(resultsYear), ". From left to right the bars are for each stage of the care and treatment cascade: living with HIV, diagnosed, recieving ART, and suppressed virus."))

```

`r if (asrCascades) figs("plotbarAllStates")`
...
if (asrCascades) {
  ...
  if (excludeCare) {
    ...
    finalTable <- within(finalTable, stage <- factor(stage,
      levels(stage)[c(1, 3, 2, 4)]))  
  } else {
  ...
  # Caption
  tabs("finalTable", paste0("The HIV care and treatment cascade estimates for each State and Territory in", toString(resultsYear), ". The table shows the cascade estimates with ranges in brackets for each State and Territory."))
}
```

`r if (asrCascades) tabs("finalTable")`
...
if (asrCascades) {
  ...
  # pops <- c("non-indigenous", "indigenous", "bornssa",
  # "bornsea","othercob")
  pops <- c("ausnonindigenous", "indigenous", "ssa", "sea","sca","othercob")
  ...
  popResults <- filter(hivCascade, population %in% pops, 
    year == resultsYear,
    stage != "linked", stage != "retained")
  
  # Create the plots
  plotModeBar <- ggplot(data = expResults, aes(x = population, y = value,
    fill = stage)) + 
  ...
  plotPopBar <- ggplot(data = popResults, aes(x = population, y = value,
    fill = stage)) + 
    geom_bar(position = "dodge", stat = "identity") + 
    geom_errorbar(aes(ymin = lower, ymax = upper), 
      position = position_dodge(width = 0.9),
  ...
  # Save if required
  if (savePlots) {
    png(file.path(figFolder, paste("HIVcascade-Modes-",
      toString(resultsYear), ".png",sep ="")), 
      width = 6, height = 4, units = "in", res = 300)
    print(plotModeBar)
    dev.off()
    
    png(file.path(figFolder, paste("HIVcascade-Pops-",
      toString(resultsYear), ".png",sep ="")), 
      width = 6, height = 4, units = "in", res = 300)
    print(plotPopBar)
    dev.off()
  ...
  # Caption
  figs("plotModeBar", paste0("Number of people living with HIV by mode of exposure in ", toString(resultsYear), "."))
  
  figs("plotPopBar", paste0("Number of people living with HIV by country of birth in ", toString(resultsYear), "."))
}
```

`r if (asrCascades) figs("plotModeBar")`
...
if (asrCascades) {
  ...
  finalTable$population <- as.character(finalTable$population)
  finalTable$stage <- as.character(finalTable$stage)
  ...
  finalTable[finalTable$population == "hetero",]$population <-
    "Heterosexual"
...
# Caption
tabs("finalTableExp", paste0("Number of people living with HIV by exposure population in ", toString(resultsYear), "."))

```

`r if (asrCascades) tabs("finalTableExp")`
```{r inserttable3, include = TRUE, results = "asis"}
if (asrCascades) {
  kable(finalTableExp, align = c("l", "l", "c", "c"), row.names = FALSE)
}
```

`r if (asrCascades) figs("plotPopBar")`
...
if (asrCascades) {
  ...
  # Rename second column
  finalTable[finalTable$population == "ausnonindigenous",]$population <-
    "Australian born non-indigenous"
  finalTable[finalTable$population == "indigenous",]$population <- 
    "Australian born indigenous"
  finalTable[finalTable$population == "sea",]$population <- 
    "Born in South East Asia"
  finalTable[finalTable$population == "ssa",]$population <- 
  ...
  # Caption
  tabs("finalTablePop", paste0("Number of people living with HIV by country of birth in ", toString(resultsYear), "."))
}
```

`r if (asrCascades) figs("finalTablePop")`
```{r inserttable4, include = TRUE, results = "asis"}
if (asrCascades) {
  kable(finalTablePop, align = c("l", "l", "c", "c"), row.names = FALSE)
}
```
...
if (asrCascades) {
  ...
  plotBarMale <- ggplot(data = filter(genderAllYear, 
    population == "male"), aes(x = stage, y = value)) + 
    geom_bar(stat = "identity", color = asrcols, 
      fill = asrcols, width = 0.8) +
    geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1, 
      color = "black", size = 1.1) +
  ...
  # Caption
  figs("plotBarGender", paste0("National HIV cascades for males and females
    in ", toString(resultsYear), "."))
...
if (asrCascades) {
  ...
  # Caption
  tabs("genderTable", paste0("Estimates and range for each stage of the HIV care and treatment cascade for Australian males and feamles in ", toString(resultsYear), "."))
}
```

`r if (asrCascades) figs("plotBarGender")`
...
`r if (asrCascades) tabs("genderTable")`
```{r inserttable5, include = TRUE, results = "asis"}
if (asrCascades) {
  kable(genderTable, align = c("l", "c", "c"), row.names = FALSE)
}
```
