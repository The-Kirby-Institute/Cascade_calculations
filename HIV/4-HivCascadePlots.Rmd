...
# Script parameters ------------------------------------------------------
resultsYear <- 2021  # year of cascade we will plot
minYear <- 18 # how long ago do we plot things (from 2004)
startYear <- resultsYear - minYear + 1 

# Set up project - If ASR plot all populations otherwise just plot 
# specified cascade
projectName <- "State_Cascades_2022" #"ASR_2022" "HIV_Cascade_Paper"
resultsFolder <- file.path(resultsFolder, projectName)
cascadePop <- "all"

savePlots <- TRUE # save plots as separate files
pubPlots <- FALSE   # create additional plots for publications
...
if (projectName %in% c("ASR_2022", "ASR_2021", "ASR_2020", "ASR_2019", "ASR_2018", 
  "ASR_2017", "ASR_2016")) {
...
if (pubPlots) {
  ...
  plotDeaths <- ggplot(data = hivResults, aes(x = yeardiagnosis, 
    ...
    expand_limits(y = 0) +
    scale_y_continuous(labels = comma) + 
    ylab("Number of deaths among diagnosed") + xlab("Year") +  
    PlotOptions()
  ...
  plotMigrants <- ggplot(data = hivResults, aes(x = yeardiagnosis, 
    ...
    expand_limits(y = 0) +
    scale_y_continuous(labels = comma) + 
    ylab("Number of emmigrants among diagnosed") + xlab("Year") +  
    PlotOptions()
