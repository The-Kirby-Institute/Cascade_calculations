...
minYear <- 20 # how long ago do we plot things (from 2004)
startYear <- resultsYear - minYear + 1 

# Set up project - If ASR plot all populations otherwise just plot 
# specified cascade
projectName <- "ASR_2025" #"ASR_2025" "HIV_Cascade_Paper"
...
if (pubPlots) {
  ...
  hivResults$emigrants <- cumsum(pldhivAll$emigrants + pldhivAll$diag_departs)
  hivResults$emigrants_min <- cumsum(pldhivMin$emigrants + pldhivMin$diag_departs)
  hivResults$emigrants_max <- cumsum(pldhivMax$emigrants + pldhivMax$diag_departs)
...
if (pubPlots) {
  ...
  plotMigrants <- ggplot(data = hivResults, aes(x = yeardiagnosis, 
    ...
    expand_limits(y = 0) +
    scale_y_continuous(labels = comma) + 
    ylab("Number of emigrants") + xlab("Year") +  
    PlotOptions()
  ...
  plotMigrationRate <- ggplot(data = hivResults, 
    ...
    ylab("Emigration rate (%)") + xlab("Year") +  
    PlotOptions()
