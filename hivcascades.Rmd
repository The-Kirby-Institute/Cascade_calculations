...
# Using VL estimates
avevlTests <- filter(ahodTests, state == "all")$avevltests
numvlTests <- filter(vlnoARTdata, state == "all" & year %in% treatYears)$n

receivedCareVL <- (numvlTests/avevlTests)

# Using CD4 data
numTreated <- filter(hivtreated, population == "all")$value
aveCd4_Rx <- filter(ahodTests, state == "all")$avecd4Rxtests
aveCd4_noRx <- filter(ahodTests, state == "all")$avecd4tests
numCd4Tests <- filter(cd4data, state == "all" & year %in% treatYears)$n

receivedCareCD4 <- (numCd4Tests - numTreated * aveCd4_Rx)/aveCd4_noRx
...
for (ii in 1:nstates) {
  ...
  numTests <- filter(vlnoARTdata, state == states[ii] & year %in% treatYears)$n

  if (states[ii] %in% availableStates) {
     avevlTests <- filter(ahodTests, state == states[ii])$avetests
    if (any(is.na(avevlTests))){
      naindex <- is.na(avevlTests)
      tempTests <- filter(ahodTests, state == "all")$avetests
      avevlTests[naindex] <- tempTests[naindex]
    }     
  } else {
    avevlTests <- filter(ahodTests, state == "all")$avetests
  }

  receivedCare <- (numTests/avevlTests)
