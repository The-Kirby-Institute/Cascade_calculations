...
hivaborig[!is.na(hivaborig$aboriggroup),]$propknown <- hivaborigknown$propnotifications
hivaborig[is.na(hivaborig$aboriggroup),]$propknown <- 0
hivaborig$proptotalknown <- NA
hivaborig[!is.na(hivaborig$aboriggroup),]$proptotalknown  <- hivaborigknown$proptotalnotifications
hivaborig[is.na(hivaborig$aboriggroup),]$proptotalknown <- 0

# Create some useful plots
if (plots) {
  # ggplot(hivtemp,aes(x = yeardiagnosis, y = notifications, fill = countrygroup)) + geom_area()
  # ggplot(hivcob, aes(x = yeardiagnosis, y = propnotifications, fill = countrygroup)) + geom_area()
  # ggplot(hivaborig, aes(x = yeardiagnosis, y = propnotifications, fill = aboriggroup)) + geom_area()
} 
...
#   expmrate1 <- predict(lmrate, data.frame(year = allyears))
  ...
  allPredicts <- rbind(allPredicts, data.frame(year = allyears, 
                                state = rep(states[ii],length(allyears)),
                                rate = expmrate[, 1], 
                                lower = expmrate[, 2],
                                upper = expmrate[, 3]))
  #   allPredicts[[states[ii]]] <- expmrate1
}
...
if (plots) {
  ...
  mplot3 <- ggplot(data = filter(hivmigrate, state == "all"), aes(x = year, y = rate)) +
    ...
    ggtitle("Fitted annual migration rate (National with 90% CI)") + theme_bw() +
    opts
...
# Initialize final results data frame
hivtreated <- data.frame(stage = character(), 
                         year = double(),
                         population = character(),
                         value = double(),
                         lower = double(),
...
treatyears <- (analysisYear - 10):2013
...
### Now use the relative rates to calculate number om treatment
treated <- prospectionData %>%
  filter(gender == "all") %>%
  select(-gender) 
colnames(treated) <- c("population","2013","2014")
treated <- gather(treated,"year","value",2:3)
treated$year <- as.numeric(as.character(treated$year))
...
for (ii in 1:nstates) {
  # Extract 2013 data 
  treat2013 <- filter(treated, year == 2013 & state == states[ii])$numART
  ...
  # Create a data frame 
  tempdf <- data_frame(state = states[ii], year = treatyears, numART = tempfit,
                       lower = templwr, upper = tempupr)
  
  # Append data prior to 2013
  treated <- rbind(treated,filter(tempdf, year < 2013))
}

# Arrange by state and year
treated <- arrange(treated, state, year)
treated$stage <- "numART"


# Finally bind to overall results
if (bindCascade) {
  hivcascade <- rbind(hivcascade,treated)
}
...
if (plots) {
  tplot1 <- ggplot(data = relTreat, aes(x = year, y = fit)) + 
    ...
    ylim(c(0,1.5)) + xlim(c(2003,2013)) + 
    ylab("Relative number on treatment") + xlab("Year") + 
    ggtitle("Fitted number on ART relative to 2012") + 
    theme_bw() + opts
  
  tplot2 <- ggplot(data = filter(treated, state == "all"), aes(x = year, y = numART)) +
    geom_line(color = "red") + geom_line(aes(y = lower), color = "blue", linetype = "dashed") + 
    geom_line(aes(y = upper), color = "blue", linetype = "dashed") + 
    geom_point(data = filter(treated, state == "all" & year >= 2013), color = "black", size = 4) +
    ylim(c(0,NA)) + 
