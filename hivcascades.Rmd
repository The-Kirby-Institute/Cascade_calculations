...
ahodTests <- ahodData %>% 
  group_by(year, state) %>%
  summarise(ntests = sum(meanVL_noRx * n_id),
            npeople = sum(n_id),
            avetests = ntests/npeople,
            prop_notests = sum(Cd4noRxVL)/npeople) %>%
  filter(year <= analysisYear)
...
numTests <- filter(vlnoARTdata, state == "all" & year %in% treatYears)$n
