...
# By state
numberUniqueAct <- numUnique(dobact, unique(dobact$yeardiagnosis), ignore)
numberUniqueNsw <- numUnique(dobnsw, unique(dobnsw$yeardiagnosis), ignore)
numberUniqueVic <- numUnique(dobvic, unique(dobvic$yeardiagnosis), ignore)
numberUniqueQld <- numUnique(dobqld, unique(dobqld$yeardiagnosis), ignore)
numberUniqueNt <- numUnique(dobnt, unique(dobnt$yeardiagnosis), ignore)
numberUniqueWa <- numUnique(dobwa, unique(dobwa$yeardiagnosis), ignore)
numberUniqueSa <- numUnique(dobsa, unique(dobsa$yeardiagnosis), ignore)
numberUniqueTas <- numUnique(dobtas, unique(dobtas$yeardiagnosis), ignore)

# By mode
numberUniqueMsm <- numUnique(dobmsm, unique(dobmsm$yeardiagnosis), ignore)
numberUniquePwid <- numUnique(dobpwid, unique(dobpwid$yeardiagnosis), ignore)
numberUniqueHetero <- numUnique(dobhetero, unique(dobhetero$yeardiagnosis), ignore)
numberUniqueOther <- numUnique(dobother, unique(dobother$yeardiagnosis), ignore)
...
hivstate[hivstate$state == "act",]$uniquecases <- numberUniqueAct
hivstate[hivstate$state == "nsw",]$uniquecases <- numberUniqueNsw
hivstate[hivstate$state == "vic",]$uniquecases <- numberUniqueVic
hivstate[hivstate$state == "qld",]$uniquecases <- numberUniqueQld
hivstate[hivstate$state == "nt",]$uniquecases <- numberUniqueNt
hivstate[hivstate$state == "wa",]$uniquecases <- numberUniqueWa
hivstate[hivstate$state == "sa",]$uniquecases <- numberUniqueSa
hivstate[hivstate$state == "tas",]$uniquecases <- numberUniqueTas

# Store exposure results
hivexpnat$uniquecases <- rep(NA,nrow(hivexpnat))

hivexpnat[hivexpnat$expgroup == "hetero",]$uniquecases <- numberUniqueHetero
hivexpnat[hivexpnat$expgroup == "msm",]$uniquecases <- numberUniqueMsm
hivexpnat[hivexpnat$expgroup == "other",]$uniquecases <- numberUniqueOther
hivexpnat[hivexpnat$expgroup == "pwid",]$uniquecases <- numberUniquePwid
