...
for (ii in 1:nstates) {
  ...
  if (useCD4) { 
    ...
    if (states[ii] %in% availableStates) {
      ...
      aveCd4_Rx <- filter(ahodTests, state == states[ii])$avecd4Rxtests
      if (any(is.na(aveCd4_Rx))){
        naindex <- is.na(aveCd4_Rx)
        temp <- filter(ahodTests, state == "all")$avecd4Rxtests
        aveCd4_Rx[naindex] <- temp[naindex]
      }
      
      aveCd4_noRx <- filter(ahodTests, state == states[ii])$avecd4tests
      if (any(is.na(aveCd4_noRx))){
        naindex <- is.na(aveCd4_noRx)
        temp <- filter(ahodTests, state == "all")$avecd4tests
    ...
    if (states[ii] %in% availableStates) {
      avevlTests <- filter(ahodTests, state == states[ii])$avevltests
      if (any(is.na(avevlTests))){
        naindex <- is.na(avevlTests)
        tempTests <- filter(ahodTests, state == "all")$avevltests
