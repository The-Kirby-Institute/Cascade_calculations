...
if (nationalUniques) {
  hivstate$propunique <- hivresults$propunique
} else {  
...
# Calculate proportion unique
if (nationalUniques) {
  hivexpnat$propunique <- hivresults$propunique
} else {
...
for (ii in 1:npops) {
  ...
  # Do the Pop calculation and range
  pldhivPop <- livingDiagnosed(cumPopDiags, deathRate, migrationRate)
  
  pldhivPopMin <- livingDiagnosed(cumPopDiags, deathRateMax, migrationRateMax)
  pldhivPopMax <- livingDiagnosed(cumPopDiags, deathRateMin, migrationRateMin)
...
for (ii in 1:nstates) {
  ...
  # Create a data frame 
  tempdf <- data_frame(population = states[ii], year = treatyears, value = tempfit,
