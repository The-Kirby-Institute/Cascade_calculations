...
permDeparts <- TRUE # Use national permant departures for overseas migration
...
deathrate[(indexLinkage+1):length(deathrate)] <- filter(ahodDeaths, 
                                                        state == "all")$deathrate # ahodMortality
...
deathrate_min[(indexLinkage+1):length(deathrate)] <- filter(ahodDeaths, 
                                                        state == "all")$lower # ahodMortalityLower
...
deathrate_max[(indexLinkage+1):length(deathrate)] <- filter(ahodDeaths, 
                                                        state == "all")$upper # ahodMortalityUpper
...
if (useOldpropundiagnosed) {
  # Replace 2014 estimates with COUNT and 2014 ASR supplement non-MSM estimates
  # COUNT: 8.9% (95% CI: 5.8-13.5%)
  # Non-MSM: 20% (range 12-23%) across states

  hardcode <- c(analysisYear, 0.09, 0.06, 0.14, 0.2, 0.15, 0.25)
  
  propUndiagnosed[propUndiagnosed$year == analysisYear,] <- hardcode
}
...
if (useCD4) {
  # Using CD4 data
  numTreated <- filter(hivtreated, population == "all")$value
  aveCd4_Rx <- filter(ahodTests, state == "all")$meanCd4_Rx
  aveCd4_noRx <- filter(ahodTests, state == "all")$meanCd4_noRx
  ...
  # Use VL estimates
  avevlTests <- filter(ahodTests, state == "all")$meanVL_noRx
...
for (ii in 1:nstates) {
  ...
  numTreated <- filter(hivtreated, population == states[ii])$value
  numCd4Tests <- filter(cd4data, state == states[ii] & year %in% treatYears)$n
  ...
  if (useCD4) { 
    ...
    if (states[ii] %in% availableStates) {
      ...
      aveCd4_Rx <- filter(ahodTests, state == states[ii])$meanCd4_Rx
      if (any(is.na(aveCd4_Rx))){
        naindex <- is.na(aveCd4_Rx)
        temp <- filter(ahodTests, state == "all")$meanCd4_Rx
        aveCd4_Rx[naindex] <- temp[naindex]
      }
      
      aveCd4_noRx <- filter(ahodTests, state == states[ii])$meanCd4_noRx
      if (any(is.na(aveCd4_noRx))){
        naindex <- is.na(aveCd4_noRx)
        temp <- filter(ahodTests, state == "all")$meanCd4_noRx
    ...
    # Use VL testing data 
    numvlTests <- filter(vlnoARTdata, state == states[ii] & year %in% treatYears)$n
  
    # Work out average VL tests for each state if available and non-NA
    if (states[ii] %in% availableStates) {
      avevlTests <- filter(ahodTests, state == states[ii])$meanVL_noRx
      if (any(is.na(avevlTests))){
        naindex <- is.na(avevlTests)
        tempTests <- filter(ahodTests, state == "all")$meanVL_noRx
