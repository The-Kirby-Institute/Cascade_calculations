...
for (ii in 1:nstates) {
  ...
  treatFinal <- filter(hivTreated, population == states[ii],
                       year >= 2013)
  ...
  # Create a data frame 
  tempDf <- data_frame(population = states[ii], year = 2000:analysisYear,
                       value = tempFit, lower = tempLwr, upper = tempUpr, 
                       stage = "numART")
  
  # Append data prior to 2013 removing old 2013 and 2014 rows
  tempTreated <- filter(hivTreated, population != states[ii])
...
for (region in states) {
  ...
  # Number treated 
  tempTreat <- filter(hivTreated, population == region)$value
  tempTreatMin <- filter(hivTreated, population == region)$lower
  tempTreatMax <- filter(hivTreated, population == region)$upper
