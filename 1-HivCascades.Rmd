...
if (permDeparts) {
  ...
  mrate <- (absDeparts$rate_permanent + absDeparts$overall_rate) / 2
...
ahodData <- read.csv(file.path(dataFolder, 
                               paste("ahod", toString(tempYear), 
                                     ".csv", sep = "")))
temp2015 <- filter(ahodData, year == 2014) %>%
  mutate(year = 2015)
ahodData <- bind_rows(ahodData, temp2015)
