...
if(cascadeImpact) {
  ...
  deathRateFix[start:end] <- deathRate[start]
  
  deathRateFix_min <- deathRate_min
  deathRateFix_min[start:end] <- deathRate_min[start]
  
  deathRateFix_max <- deathRate_max
  deathRateFix_max[start:end] <- deathRate_max[start]
...
if(cascadeImpact) {
  ...
  pldhivFixMin <- LivingDiagnosed(cumDiagnoses, deathRateFix_min,
                                  migrationRate)
  
  pldhivFixMax <- LivingDiagnosed(cumDiagnoses, deathRateFix_max,
                                  migrationRate)
}
...
numUnknown$pwid <- filter(propExpDiags, population == "pwid")$propval *
  numUnknown$value 
numUnknown$hetero <- filter(propExpDiags, population == "hetero")$propval *
  numUnknown$value 
...
artEstimates <- artEstimates %>%
  select(year, all, fitmin, fitmax, rel, rellower, relupper) %>%
  rename(value = all, lower = fitmin, upper = fitmax, 
