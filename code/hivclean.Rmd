...
rawdata <- file.path(dataPath,paste("cascadeHIVnotifications-", toString(dataYear), ".csv",sep=""))
...
hivdata[which(hivdata$rob %in% c(1,2,3,4,5,6,8,10,11)),]$countrygroup <- "other"
hivdata[which(hivdata$rob == 7 & hivdata$cob != 1100),]$countrygroup <- "other"

hivdata$aboriggroup <- rep(NA,nrow(hivdata))
hivdata[which(hivdata$cob != 1100 | hivdata$rob != 7),]$aboriggroup <- "other"
hivdata[which(hivdata$cob == 0),]$aboriggroup <- NA
hivdata[which(hivdata$cob == 1100 & hivdata$aborig == 2),]$aboriggroup <- "indigenous"
hivdata[which(hivdata$cob == 1100 & hivdata$aborig == 1),]$aboriggroup <- "non-indigenous"
...
# Calculate the range in the proportion for Non-MSM
propNonMSMdeathsMin <- deathsNonMSM$lowerCI/(deathsNonMSM$lowerCI + deathsMSM$UpperCI)
propNonMSMdeathsMin[is.nan(propNonMSMUndiagMin)] <- NA
propNonMSMdeathsMax <- deathsNonMSM$UpperCI/(deathsNonMSM$UpperCI + deathsMSM$lowerCI)
