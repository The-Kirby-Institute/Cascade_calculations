...
This script explores the number of people taking ART in Australia. 
...
```{r loaddata}
# Load the ART data
dataFile <- paste("ART_estimates-", toString(analysisYear), 
                  ".csv", sep = "")
artEstimates <- read.csv(dataFile)
...
artEstimates$diff <- saturationCoverage - artEstimates$all 
...
ggsave("Regression_Comparision.png", plot = regressionPlot,
       width = plotWidth, height = plotHeight, units = plotUnits)
ggsave("Fit_Comparison.png", plot = fitPlot, 
       width = plotWidth, height = plotHeight, units = plotUnits)
ggsave("Logistic_Fit.png", plot = logPlot,
       width = plotWidth, height = plotHeight, units = plotUnits)
