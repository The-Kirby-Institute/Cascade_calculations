...
figuresFolder <- file.path(resultsFolder,"figures", "hiv_cascade")
...
# Calculate relative size of plhiv if deathrate stays high
...
print(relPopBase)
# Estimate upper and lower bounds of baseline population
plhivLowerBase <- relPopBase * plhivLower


plhivUpperBase <- relPopBase * plhivUpper
...
for (ii in 1:samples) {
  ...
  for (jj in 2:nyears) {
    ...
    # plhivSampleBase <- runif(1, plhivLowerBase[jj], plhivUpperBase[jj])
    plhivSampleBase <- relPopBase[jj] * plhivSample
    # plhivSampleBase <- plhivSample
...
incPlot <- ggplot(data = results, aes(x = year)) + 
  ...
  scale_colour_manual(name = "", 
                      ...
                      labels = c("ART expansion and IQR", 
                                 "Annual notifications", 
                                 "Maintain 2005 ART coverage"),
