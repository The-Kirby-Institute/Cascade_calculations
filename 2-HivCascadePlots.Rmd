...
if (pubPlots) {
  ...
  expDiags <- hivSet %>% 
    ...
    filter(yeardiagnosis <= resultsYear, yeardiagnosis >= startYear - 1) %>%
  ...
  # Create the plots - plot after 1984 to exclude small numbers early in the 
  # epidemic
  ...
  linkDays <- hivSet %>%
    ...
    mutate(dayslinked = as.numeric(as.Date(cd4date) - as.Date(datediagnosis))) %>%
  ...
  linkDist <- ggplot(data = linkDays, aes(x = dayslinked)) + 
    geom_freqpoly(binwidth = 7, aes(colour = "all")) + # bin days into weeks
...
for (ii in seq(along = states)){
  ...
  if (states[ii] == "nsw" & excludeCare) {
    ...
    plotbarState <- ggplot(data = resultsStatesYear, 
                           aes(x = stage, y = value)) + 
      geom_bar(stat = "identity", color = maincols[2], 
               fill = maincols[2], width = 0.6) +
      geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1, 
