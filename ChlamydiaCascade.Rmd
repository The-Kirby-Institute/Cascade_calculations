...
notificationsFile <- file.path(dataFolder,
  paste("chlamnotifications", toString(resultsYear), ".csv", sep = ""))
...
chlamcascade <- rbind(chlamcascade, 
                      data.frame(stage = "notifications",
                        ...
                        percentprevious = 100 * c(maleDiags / maleInc,
...
maleTreated <- filter(chlamcascade, stage == "notifications", 
  sex == "male")$estimate * c(propTreated, propTreatedMin, propTreatedMax)

femaleTreated <- filter(chlamcascade, stage == "notifications", 
  sex == "female")$estimate * c(propTreated, propTreatedMin,
  propTreatedMax)
...
chlamcascade <- rbind(chlamcascade, 
                  data.frame(stage = "treated",
                    ...
                    percentprevious = 100 * c(maleTreated[1] / maleDiags,
...
maleRetested <- as.numeric(filter(chlamcascade, stage == "treated",
  sex == "male")[, c("estimate", "lower", "upper")] * 
  c(propReTestMale, propReTestMaleLower,
                              propReTestMaleUpper))

femaleRetested <- as.numeric(filter(chlamcascade, stage == "treated",
  sex == "female")[, c("estimate", "lower", "upper")] * 
  c(propReTestFemale, propReTestFemaleLower,
...
chlamcascade <- rbind(chlamcascade, data.frame(stage = "retested",
              ...
              percentprevious = 100 * c(maleRetested[1] / maleTreated[1],
                                  femaleRetested[1] / femaleTreated[1])))
...
`r 100*round(propReTest, 3)`% of people diagnosed with chlamydia are 
...
`r 100*round(sum(propPositive * weights), 3)`% of people were positive at 
their retest overall. This means `r 100*round(propNegative, 3)`% of 15-29 
