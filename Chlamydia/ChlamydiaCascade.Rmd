...
fileType <- ".png" # ".png" for documents; .tiff for paper
retestRange <- TRUE # Calculate variations in retested
msmSplit <- TRUE # For determining which retest variation file to use
...
fmStr <- FrontMatter(packages = "all", rstudio = "1.1.423")
cat(fmStr)
```
...
if (retestRange) {
  ...
  if (msmSplit) {
    retestVarFile <- file.path(dataFolder, paste0("chlamRetestVariation",
      toString(tail(resultsYear, 1)), "_MSM.csv"))
  } else {
    retestVarFile <- file.path(dataFolder, paste0("chlamRetestVariation",
  ...
  retests <- read.csv(retestVarFile, stringsAsFactors = FALSE) %>%
    filter(sex %in% c("all", "females", "males")) %>%
...
cols <- PlotColors("sticascade")
asrcols <- c("males" = cols[1], "females" = cols[2]) 
```
