...
currentDiags <- notifications %>% 
  filter(year %in% c((resultsYear-2):resultsYear)) %>%
...
chlamcascade <- rbind(chlamcascade, 
  data.frame(stage = "infections",
    year = rep((resultsYear-2):resultsYear, 3),
...
propTreatedUrban <- tibble(year = (resultsYear-2):resultsYear,
  region = "urban", 
...
propTreatedRegion <- tibble(year = (resultsYear-2):resultsYear, 
  region = "regional", 
  proportion = propTreatedRegion)

propTreatedRemote <- rowMeans(matrix(filter(treatedDF, 
  region == "remote")$proportion, nrow = 3, byrow = TRUE))
propTreatedRemote <- tibble(year = (resultsYear-2):resultsYear, 
  region = "remote",
...
if (saveCascade) {
  ...
  saveString2 <- paste0("chlamydia_cascade_trend-", resultsYear-2, "-", resultsYear, ".csv")
