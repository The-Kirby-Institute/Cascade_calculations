...
# Script parameters
resultsYear <- 2024 # year of cascade we will calculate and plot   
dataYear <- 2024 # year data for estimates was generated (to handle updated 
                 # data) this is the data output year as well.
numyears <- 5 # Number of years of cascades
...
currentDiags <- notifications %>% 
  filter(year %in% c((resultsYear-numyears+1):resultsYear)) %>%
...
currentTreat <- treatment %>%
  dplyr::select(year, stage, region, context, sex, proportion, prop_lower, prop_upper)
...
chlamcascade <- data.frame(year = numeric(), stage = character(), sex = character(),
...
chlamcascade <- rbind(chlamcascade, 
  data.frame(stage = "infections",
    year = rep((resultsYear- numyears + 1):resultsYear, 3),
    sex = rep(c("male", "female", "all"), 
      each = length((resultsYear - numyears + 1):resultsYear)),
...
weights <- dplyr::select(weightsDF, year, region, proportion) %>%
  rename(weight = proportion)
...
weightsClinic <- dplyr::select(weightsClinicDF, year, context, proportion)
...
propTreatedUrban <- filter(weightsClinic, context == "gp")$proportion * 
  filter(treatedDF, region == "urban", context == "gp")$proportion + 
  (1 - filter(weightsClinic, context == "gp")$proportion) * 
  filter(treatedDF, region == "urban", context == "other")$proportion
propTreatedUrban <- tibble(year = (resultsYear - numyears + 1):resultsYear,
  region = "urban", 
  proportion = propTreatedUrban)

propTreatedRegion <- filter(weightsClinic, context == "gp")$proportion * 
  filter(treatedDF, region == "regional", context == "gp")$proportion + 
  (1 - filter(weightsClinic, context == "gp")$proportion) * 
  filter(treatedDF, region == "regional", context == "other")$proportion
propTreatedRegion <- tibble(year = (resultsYear - numyears + 1):resultsYear, 
  region = "regional", 
  proportion = propTreatedRegion)

propTreatedRemote <- rowMeans(matrix(filter(treatedDF, 
  region == "remote")$proportion, nrow = numyears, byrow = TRUE))
propTreatedRemote <- tibble(year = (resultsYear - numyears + 1):resultsYear, 
  region = "remote",
  proportion = propTreatedRemote)

propTreated <- bind_rows(propTreatedUrban, propTreatedRegion) %>%
  bind_rows(propTreatedRemote) %>%
  left_join(weights, by = c("year", "region")) %>%
  arrange(year) 

propTreated <- propTreated %>%
  group_by(year) %>%
  summarise(proportion = sum(proportion * weight))

# Hard coded ranges based on propTreated value - +/- 10%
propTreatedMin <- filter(treatedDF, region == "remote") %>% 
  group_by(year) %>% 
  summarise(proportion = min(proportion)) # minimum in remote
propTreatedMax <- propTreated %>%
  mutate(proportion = min(1, proportion * 1.1))
...
maleTreated <- maleDiagnoses %>%
  select(year, estimate, lower, upper) %>%
  mutate(estimate = estimate*propTreated$proportion,
    lower = lower*propTreatedMin$proportion,
    upper = upper*propTreatedMax$proportion) %>%
  mutate(stage = "treated", 
    sex = "male",
    percentprevious = estimate / maleDiags,
    percentpreviousLower = lower / upperMaleDiags,
    percentpreviousUpper = upper / lowerMaleDiags) %>%
  select(stage, year, sex, estimate, lower, upper, 
    everything())

femaleTreated <- femaleDiagnoses %>%
  select(year, estimate, lower, upper) %>%
  mutate(estimate = estimate*propTreated$proportion,
    lower = lower*propTreatedMin$proportion,
    upper = upper*propTreatedMax$proportion) %>%
  mutate(stage = "treated", 
    sex = "female",
    percentprevious = estimate / femaleDiags,
    percentpreviousLower = lower / upperFemaleDiags,
    percentpreviousUpper = upper / lowerFemaleDiags) %>%
  select(stage, year, sex, estimate, lower, upper, 
    everything())

allTreated <- tibble(
  stage = "treated", year = maleTreated$year, sex = "all") %>%
  mutate(estimate = maleTreated$estimate + femaleTreated$estimate,
    lower = maleTreated$lower + femaleTreated$lower,
    upper = maleTreated$upper + femaleTreated$upper) %>%
  mutate(percentprevious = estimate / (maleDiags + femaleDiags),
    percentpreviousLower = lower / (upperMaleDiags + upperFemaleDiags),
    percentpreviousUpper = upper / (lowerMaleDiags + lowerFemaleDiags)) %>%
  select(stage, year, sex, estimate, lower, upper, 
    everything())

chlamcascade <- chlamcascade %>%
  bind_rows(maleTreated) %>%
  bind_rows(femaleTreated) %>%
  bind_rows(allTreated)
...
diagnosesProp <- currentTreat %>%
  filter(stage == "diagnoses") %>%
  select(year, context, proportion) %>%
  spread(context, proportion) %>%
  mutate(non_shc = gp + other) %>%
  select(year, gp= non_shc, shc)

# Functions for proportion retested
propRetested <- function(gender, value) {
  # Local function to calculate proportion tested
  propReTest <- filter(weights, region == "urban")$weight * 
      (diagnosesProp$shc * filter(retestDF, sex == gender, region == "urban", 
        context == "shc")[value] + 
      diagnosesProp$gp * filter(retestDF,  sex == gender, region == "urban", 
        context == "gp")[value]) + 
      filter(weights, region == "regional")$weight * 
      (diagnosesProp$shc * filter(retestDF, sex == gender, region == "regional", 
            context == "shc")[value] + 
          diagnosesProp$gp * filter(retestDF,  sex == gender, region == "regional", 
            context == "gp")[value]) + 
      filter(weights, region == "remote")$weight * 
      (diagnosesProp$shc * filter(retestDF, sex == gender, region == "remote", 
            context == "shc")[value] + 
          diagnosesProp$gp * filter(retestDF,  sex == gender, region == "remote", 
            context == "gp")[value])
  
  return(tibble(year = diagnosesProp$year, proportion = propReTest[[value]]))
}
...
maleRetested <- chlamcascade %>%
  filter(stage == "treated", sex == "male") %>%
  select(year, sex, estimate, lower, upper) %>%
  mutate(estimate = estimate * propReTestMale$proportion,
    lower = lower * propReTestMaleLower$proportion,
    upper = upper * propReTestMaleUpper$proportion,
    stage = "retested") %>%
  select(stage, year, sex, estimate, lower, upper, 
    everything())

femaleRetested <- chlamcascade %>%
  filter(stage == "treated", sex == "female") %>%
  select(year, sex, estimate, lower, upper) %>%
  mutate(estimate = estimate * propReTestFemale$proportion,
    lower = lower * propReTestFemaleLower$proportion,
    upper = upper * propReTestFemaleUpper$proportion,
    stage = "retested") %>%
  select(stage, year, sex, estimate, lower, upper, 
    everything())

allRetested <- tibble(stage = "retested", 
  year = maleRetested$year,
  sex = "all", 
  estimate = maleRetested$estimate + femaleRetested$estimate, 
  lower = maleRetested$lower + femaleRetested$lower, 
  upper = maleRetested$upper + femaleRetested$upper)
...
chlamcascade <- chlamcascade %>%
  bind_rows(maleRetested) %>%
  bind_rows(femaleRetested) %>%
  bind_rows(allRetested)

updateTreated <- chlamcascade %>% 
  filter(stage == "treated") %>%  
  mutate(percentprevious = 100 * estimate / filter(chlamcascade, 
    stage == "notifications")$estimate,
    percentpreviousLower = 100 * lower / filter(chlamcascade, 
      stage == "notifications")$upper,
    percentpreviousUpper = 100 * upper / filter(chlamcascade, 
      stage == "notifications")$lower)

updateRetested <- chlamcascade %>% 
  filter(stage == "retested") %>%  
  mutate(percentprevious = 100 * estimate / filter(chlamcascade, stage == "treated")$estimate,
    percentpreviousLower = 100 * lower / filter(chlamcascade, stage == "treated")$upper,
    percentpreviousUpper = 100 * upper / filter(chlamcascade, stage == "treated")$lower)

chlamcascade <- chlamcascade %>%
  filter(stage %in% c("infections", "notifications")) %>%
  bind_rows(updateTreated) %>%
  bind_rows(updateRetested)

# Store in our cascade
# chlamcascade <- rbind(chlamcascade, 
#   data.frame(stage = "retested",
#     year = rep((resultsYear-2):resultsYear, 
#       each = length((resultsYear-2):resultsYear)),
#     sex = rep(c("male", "female", "all"), 
#       each = length((resultsYear-2):resultsYear)),
...
if (saveCascade) {
  ...
  # Cascade
  saveString1 <- paste0("chlamydia_cascade-", resultsYear, ".csv")
  saveString2 <- paste0("chlamydia_cascade_trend-", resultsYear - numyears + 1, 
    "-", resultsYear, ".csv")
  
  
  write.csv(filter(chlamcascade, year == resultsYear), file = file.path(resultsFolder, saveString1), 
    row.names = FALSE)
  
   write.csv(chlamcascade, file = file.path(resultsFolder, saveString2), 
    row.names = FALSE)
...
plotcascade <- filter(chlamcascade, year == resultsYear)
...
allCascade <- ggplot(data = filter(cascadeAll, sex %in% c("male", "female"),
  ...
  geom_text(aes(label = ifelse(is.na(percentprevious), "",
    paste0(format(percentprevious, digits = 2), "%")),
    y = texty + 10000),
    position = position_dodge(width = 0.9),
    fontface = "bold",
    size = 2.5) +
