...
chlamRetest <- read_excel(rawDataTreatment, sheet = 5, skip = 3,
                          col_names = FALSE)
...
# Raw testing file
rawDataTreatment <- file.path(dataPath, paste0("accept_cascade_data-", 
                                    toString(dataYear), ".xlsx"))
...
```{r restesting NSW GP}
# Read in NSW GP data from ACCESS and clean it up 
dataStartYear <- 2007

# Raw testing file
rawDataTreatment <- file.path(dataPath, paste0("access_nsw_gp_data-", 
                                    toString(dataYear), ".xlsx"))
...
write.csv(retestNSWgp, file = file.path(cleanDataFolder,
  paste0("retestsNSWgp", toString(dataYear), ".csv")),
          row.names = FALSE)
...
rawDataTreatment <- file.path(dataPath,
                              paste0("access_chlamydia_shc_data-", 
                                    toString(dataYear), ".xlsx"))
...
colnames(retestVariation)[3:(3 + numDataYears - 1)] <- 
  paste0(dataYears, "all")
colnames(retestVariation)[(3 + numDataYears):(3 + 2 * numDataYears - 1)] <- paste0(dataYears, "male")
colnames(retestVariation)[(3 + 2 * numDataYears):
                        (3 + 3 * numDataYears - 1)] <- paste0(dataYears,
                                                              "female")
