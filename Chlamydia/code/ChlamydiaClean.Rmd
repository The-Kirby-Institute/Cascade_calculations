...
the 
~/Evaluation\_Modelling/project\_care\_cascades/Chlamydia/data/raw
folder:
- ChlamydiaNotifications-Year.csv
- CHLAMYDIA_MBS-Year.dta

and the file in the main data folder 
...
dataPath <- file.path(basePath, "Chlamydia", "data","raw")

cleanDataFolder <- file.path(basePath, "Chlamydia", "data")
...
if (msmSplit) {
  write.csv(chlamRetest, file = file.path(cleanDataFolder,
    paste0("retestsAccess", toString(dataYear), "_MSM.csv")),
    row.names = FALSE)
} else {
  write.csv(chlamRetest, file = file.path(cleanDataFolder,
    paste0("retestsAccess", toString(dataYear), ".csv")),
    row.names = FALSE)
}
```
...
write.csv(retestAccept, file = file.path(cleanDataFolder,
  paste0("retestsAccept", toString(min(dataYear, 2014)), ".csv")),
  row.names = FALSE)
...
# Do weighted all and male calculations
msmRestest <- retestVariation %>% 
  filter(sex == "msm") %>%
  select(region, context, year, period, everything()) %>%
  rename(msm_diagnoses = diagnoses, msm_retested = retested, 
    msm_prop_retested = prop_retested, msm_prop_lower = prop_lower,
    msm_prop_upper = prop_upper) %>%
  select(-sex)
    
heteroRestest <- retestVariation %>% 
  filter(sex == "non_msm") %>%
  select(region, context, year, period, everything()) %>%
  rename(hetero_diagnoses = diagnoses, hetero_retested = retested, 
    hetero_prop_retested = prop_retested, hetero_prop_lower = prop_lower,
    hetero_prop_upper = prop_upper) %>%
  select(-sex)

femRestest <- retestVariation %>% 
  filter(sex == "female") %>%
  select(region, context, year, period, everything()) %>%
  rename(fem_diagnoses = diagnoses, fem_retested = retested, 
    fem_prop_retested = prop_retested, fem_prop_lower = prop_lower,
    fem_prop_upper = prop_upper) %>%
  select(-sex)

allWeightedRetest <- left_join(msmRestest, heteroRestest, 
    by = c("region", "context", "year", "period")) %>%
  left_join(femRestest, by = c("region", "context", "year", "period")) %>%
  mutate(male_diagnoses = msm_diagnoses + hetero_diagnoses,
    all_diagnoses = msm_diagnoses + hetero_diagnoses + fem_diagnoses) %>%
  mutate(male_retested = (msm_retested + hetero_retested) / male_diagnoses,
    male_weighted_retested = msm_diagnoses / male_diagnoses *
      msm_prop_retested + hetero_diagnoses / male_diagnoses * hetero_prop_retested)

# Write to file
# if (msmSplit) { 
#   write.csv(retestVariation, file = file.path(cleanDataFolder,
#     paste0("chlamRetestVariation", toString(dataYear), "_MSM.csv")),
#     row.names = FALSE)
# } else {
#   write.csv(retestVariation, file = file.path(cleanDataFolder,
#     paste0("chlamRetestVariation", toString(dataYear), ".csv")),
#     row.names = FALSE)
# }
